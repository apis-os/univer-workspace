...
- For invalidation, TTL, and LRU eviction, `destroy-start` is emitted synchronously after the instance leaves the cache and immediately before destruction starts. Use it to discard generation-specific state; `evicted` is the later completion event.
