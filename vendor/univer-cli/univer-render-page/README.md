...
The package creates Univer inside the page, implements render and print operations, mounts the page protocol, and provides a ready-to-use Univer / Univer Pro composition. It does not start the browser itself.

PDF preparation invokes the native print operation registered for the loaded Unit. The Render Page does not maintain a separate Unit-specific PDF renderer, so browser printing remains the source of truth for print layout and output behavior.
...
The factory must register the content, rendering, UI, Facade, and print extensions required by the target Unit. The mount function handles only shared operations, Unit lifecycle, and the page protocol; it does not add missing plugins.
...
The package does not install or launch a browser, host static directories, or provide Node.js screenshot/PDF APIs. Those responsibilities belong to
[`@univer-cli/univer-render-runtime`](../univer-render-runtime/README.md).
