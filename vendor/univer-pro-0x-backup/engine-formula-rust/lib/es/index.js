...
function cellDataFromRustValue(value) {
  ...
  if (typeof value == "number")
    return { t: CellValueType.NUMBER, v: value };
...
function serializeCellData(cellData = {}, styles, precisionAsDisplayed = !1) {
  let result = {}, rawCellData = cellData instanceof ObjectMatrix ? cellData.getMatrix() : cellData;
...
var RustEngineSyncController = class extends Disposable {
  ...
  _shouldSyncMutation(command, options) {
    if (command.type !== CommandType.MUTATION || !isRustFormulaRelevantMutation(command.id) || (command.id === "formula.mutation.set-super-table" || command.id === "formula.mutation.remove-super-table") && hasUnitId(command.params) && this._projectionManagedTableUnits.has(command.params.unitId))
      return !1;
...
import {
  CellValueType as CellValueType2,
  CommandType as CommandType2,
  createBaseFormulaTableReferenceNormalizer,
  createBaseFormulaTableNameMap,
  Disposable as Disposable2,
  ICommandService as ICommandService2,
  IUniverInstanceService,
  LocaleType,
  ObjectMatrix as ObjectMatrix2,
  setDependencies as setDependencies2,
  toDisposable,
  UniverInstanceType
} from "@univerjs/core";
import {
...
var RustSameRuntimeProjectionController = class extends Disposable2 {
  ...
  _initialize() {
    ...
    this._initialProjectionLoad = this._loadAllWorkbookProjections(), this.disposeWithMe(
      toDisposable(
        this._univerInstanceService.getTypeOfUnitAdded$(UniverInstanceType.UNIVER_SHEET).subscribe(({ unit }) => {
      ...
      toDisposable(
        this._univerInstanceService.getTypeOfUnitAdded$(UniverInstanceType.UNIVER_BASE).subscribe(({ unit }) => {
      ...
      toDisposable(
        this._univerInstanceService.getTypeOfUnitDisposed$(UniverInstanceType.UNIVER_BASE).subscribe((base) => {
  ...
  _applyBaseFieldInsertion(unitId, dirtyRanges) {
    let base = this._univerInstanceService.getUnit(unitId, UniverInstanceType.UNIVER_BASE), previous = this._baseProjectionStructures.get(unitId);
    ...
    return this._rustEngineSyncController.applyProjectionMutation({
      ...
      params: {
        ...
        tables: this._getBaseTableEntries(
          insertion.tableId,
          snapshot,
          createBaseFormulaTableNameMap(snapshot)
  ...
  _getBaseProjectionData(unitId) {
    ...
    }, formulaData = { [unitId]: {} }, formulaTableNames = createBaseFormulaTableNameMap(snapshot), normalizeFormulaTableReferences = createBaseFormulaTableReferenceNormalizer(snapshot, formulaTableNames);
    for (let table of Object.values(snapshot.tables)) {
      let fieldOrder = table.fieldOrder;
      allUnitData[unitId][table.id] = {
        cellData: new ObjectMatrix2(
  ...
  async _loadAllWorkbookProjections() {
    ...
    })), calculationData = this._formulaDataModel.getCalculateData(), formulaData = this._formulaDataModel.getFormulaData(), baseWorkbooks = this._univerInstanceService.getAllUnitsForType(UniverInstanceType.UNIVER_BASE).map(
  ...
  _createBaseTableResource(unitId) {
    let base = this._univerInstanceService.getUnit(unitId, UniverInstanceType.UNIVER_BASE);
    if (base) {
      let snapshot = base.getSnapshot(), tables = Object.values(snapshot.tables), formulaTableNames = createBaseFormulaTableNameMap(snapshot), data2 = {};
...
function toBaseRuntimeCellData(value, field) {
  ...
  if (field?.type === "attachment")
    return { v: "", t: CellValueType2.STRING };
  if (Array.isArray(value))
    return { v: value.join(", "), t: CellValueType2.STRING };
  if (field?.type === "link" && value && typeof value == "object") {
    let link = value;
    return { v: String(link.text ?? link.url ?? ""), t: CellValueType2.STRING };
  }
...
setDependencies2(RustSameRuntimeProjectionController, [
  IUniverInstanceService,
...
import {
  ...
  ICommandService as ICommandService3,
  IConfigService,
  ObjectMatrix as ObjectMatrix3,
  setDependencies as setDependencies3
} from "@univerjs/core";
import {
  ENGINE_FORMULA_PLUGIN_CONFIG_KEY,
  FormulaDataModel as FormulaDataModel2,
  ...
  ISuperTableService as ISuperTableService2,
  SetArrayFormulaDataMutation,
  SetFormulaCalculationResultMutation as SetFormulaCalculationResultMutation2
} from "@univerjs/engine-formula";
import {
  ExternalReferenceModel,
  IFormulaReferenceDataService
} from "@univerjs-pro/engine-formula";
import { Subject } from "rxjs";
var IRustCalculateFormulaService = createIdentifier(
...
var RustCalculateFormulaService = class extends Disposable3 {
  ...
  _materializeRustExternalReference(response, binding) {
    ...
    for (let responseSheet of response.sheets) {
      let current = sheets[responseSheet.sheetId], cellData = current?.cellData ?? new ObjectMatrix3();
  ...
  _buildRustInput() {
    let unitData = this._currentConfigService.getUnitData(), formulaData = this._currentConfigService.getFormulaData(), dirtyRanges = this._currentConfigService.getDirtyRanges(), config = this._configService.getConfig(ENGINE_FORMULA_PLUGIN_CONFIG_KEY);
    return {
      config: {
  ...
  async _applyRustDependencyOutput(output) {
    ...
    return (patch.arrayFormulaRange || patch.arrayFormulaCellData) && (this._formulaDataModel.setArrayFormulaRange(patch.arrayFormulaRange ?? {}), this._formulaDataModel.setArrayFormulaCellData(patch.arrayFormulaCellData ?? {}), await this._commandService.executeCommand(
      ...
      SetFormulaCalculationResultMutation2.id,
      {
...
setDependencies3(RustCalculateFormulaService, [
  IConfigService,
  IFormulaCurrentConfigService,
  IFormulaRuntimeService,
  IFormulaDependencyGenerator,
...
// src/plugin.ts
var DEFAULT_RUST_FORMULA_PLUGIN_CONFIG = {}, UniverRustFormulaEnginePlugin = class extends UniverProFormulaEnginePlugin {
  ...
  _extendFormulaExecutionDependencies(dependencies) {
    if (this._config?.useRustEngine !== !1 && !this._config?.notExecuteFormula) {
      let calculateServiceIndex = dependencies.findIndex(
        (dependency) => Array.isArray(dependency) && dependency[0] === ICalculateFormulaService
      ), rustDependencies = [
        [RustEngineSyncController],
        [RustSameRuntimeProjectionController],
        [IRustCalculateFormulaService, { useClass: RustCalculateFormulaService }],
        // eslint-disable-next-line react/no-unnecessary-use-prefix
        [ICalculateFormulaService, { useFactory: () => this._injector.get(IRustCalculateFormulaService) }]
      ];
