...
- revision CAS 与 changeset/head 原子发布；
...
- schema 版本以 `('core', 1)` 写入 `collaboration_schema_versions`，不占用
  `PRAGMA user_version`。
- changeset 标量列和 submission unique constraint 提供数据库级完整性；提交只做 revision
  CAS，不按 submission key 点查。
- 空库会自动初始化；不完整或不支持的核心 schema 会被拒绝。
