...
This README focuses on Service middleware, lifecycle, and advanced APIs. For a first integration,
assemble Transport, Endpoint, Service, and a Database Adapter as described in the
[Collaboration quick start](https://office.univer.ai/collaboration/quick-start).
...
- `getUnitLoadData()`: read the recovery data for the latest or a specific revision.
- `getUnitLoadDataWithBlocks()`: retrieve the same recovery data as `getUnitLoadData()`, together with all Sheet
  blocks referenced by the snapshot. To replay subsequent changesets into a complete snapshot at
  the target revision, use `UnitSnapshotMaterializer`.
...
- `createUnitFromSnapshot()`: create a Unit from a Protocol `ISnapshot` and its Sheet blocks.
...
| `applyChangeset` | After OT aligns the changeset to the current revision, before applying it to the pending Unit state | `userID`, `memberID`, `customData`, `request`, `changeset`, `permissionRequirements`, `currentRevision`, `attempt` | Repeated after a database revision conflict | Authorization and validation of aligned mutations |
...
`permissionRequirements` lists the standard permissions required by the transformed candidate changeset; authorization policy remains application-defined.
...
const current = await collabService.getUnitLoadData(
  {
    unitID: task.unitID,
    type: task.type,
    revision: 0,
...
const result = await collabService.submitChangeset(
  {
    changeset: {
      unitID: task.unitID,
      type: task.type,
      baseRev: current.targetRevision,
      revision: current.targetRevision + 1,
...
`sid/reqId` is the submission idempotency key. A retry keeps `unitID/type/sid/reqId`; its `baseRev` may advance only as the awaiting submission is transformed by confirmed changesets ordered before it, and must not advance past its own confirmed revision. Callers are responsible for this causal API contract. `memberID` only identifies the source of the submission. Background jobs do not need to create a WebSocket Session, and the Service does not check whether that member is online.
