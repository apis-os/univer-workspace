...
本 README 重点说明 Service middleware、生命周期和高级 API。第一次接入应先按
[协同编辑快速开始](https://office.univer.ai/zh-CN/collaboration/quick-start)
完成 Transport、Endpoint、Service 和 Database Adapter 的组装。
...
- `getUnitLoadData()`：读取最新或指定 revision 的恢复数据。
- `getUnitLoadDataWithBlocks()`：获取与 `getUnitLoadData()` 相同的恢复数据，并附带 snapshot 引用的全部
  Sheet blocks。若需将后续 changesets 重放为目标 revision 的完整 snapshot，可使用
...
- `createUnitFromSnapshot()`：从 Protocol `ISnapshot` 及其 Sheet blocks 创建 Unit。
...
| `applyChangeset` | changeset 已通过 OT 与当前 revision 对齐、应用到待提交 Unit 状态前 | `userID`、`memberID`、`customData`、`request`、`changeset`、`permissionRequirements`、`currentRevision`、`attempt` | 数据库 revision 冲突后重复 | 对齐后 mutation 的授权和校验 |
...
`permissionRequirements` 表示 transform 后 candidate changeset 所需的标准权限点；授权策略仍由应用定义。
...
`sid/reqId` 是提交幂等键。任务重试保持 `unitID/type/sid/reqId`；awaiting submission
仅可在被排在它之前的 confirmed changesets transform 后推进 `baseRev`，不能越过自身
已经 confirmed 的 revision。调用方负责遵守这一因果 API 契约。
`memberID` 只表示这次提交的来源；
