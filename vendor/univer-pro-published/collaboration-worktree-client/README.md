...
Use `client.setUnitRemoved(worktreeID, unitID, true)` to record removal from draft content merge, and pass `false` to undo it before ready. The application still owns formal document deletion and Trash behavior.
