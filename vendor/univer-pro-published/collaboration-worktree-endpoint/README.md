...
`POST /universer-api/worktrees/:worktreeID/units/:unitID/removal` accepts `{ "removed": true }` or `{ "removed": false }`. The authenticated call delegates to `setUnitRemoved` and returns the updated Worktree. Removal events also update Worktree subscribers.
