import type { PivotDataFieldDataTypeEnum, PivotDataFieldSortOperatorEnum, PivotDataFieldTypeEnum, PivotDateGroupFieldDateTypeEnum, PivotFilterTypeEnum, PivotShowAsBaseItemTypeEnum, PivotShowAsTypeEnum, PivotSubtotalTypeEnum, ST_PivotFilterOperatorEnum } from './enum';
...
export interface IPivotTableValueSortInfo {
    ...
    sortItems: string[];
    /**
     * @property {string} - The BCP 47 locale used to compare labels.
     */
    sortLocale?: string;
}
/**
...
/**
 * Represents an ordered value filter rule of the pivot table.
 *
 * Currently, a label field can have at most one value filter, so `fieldId` also identifies the rule.
 * Supporting multiple value filters for the same field requires stable rule ids and explicit evaluation-order handling.
 */
