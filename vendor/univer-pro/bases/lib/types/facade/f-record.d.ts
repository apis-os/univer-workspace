import type { BaseCellValue, FieldId, IBaseAttachment, Injector, IRecordSnapshot } from '@univerjs/core';
 ...
 * ```
 */
export declare class FBaseTableRecord extends FBaseInitialable {
    private readonly _base;
    private readonly _table;
    protected readonly _recordId: string;
    protected readonly _injector: Injector;
    private readonly _commandService;
    private readonly _permissionService;
    protected readonly _baseUnitId: string;
    protected readonly _baseTableId: string;
    constructor(_base: FBase, _table: FBaseTable, _recordId: string, _injector: Injector, _commandService: ICommandService, _permissionService: IPermissionService);
    /** @ignore */
    getBaseFacade(): FBase;
    /** @ignore */
    getTableFacade(): FBaseTable;
    /**
     * Get the record id.
     ...
     * @returns {FBaseObjectPermission} Permission facade combining the Base, Table, and Record Edit points.
     * @example
     * ```ts
     ...
     * @returns {IRecordSnapshot} The record snapshot.
     * @example
     * ```ts
     * const fBase = univerAPI.getActiveBase();
     * const fBaseTable = fBase.getTableById('table-1');
     ...
     * @returns {BaseCellValue} The cell value, or `null` if the field is empty.
     * @example
     * ```ts
     * const fBase = univerAPI.getActiveBase();
     * const fBaseTable = fBase.getTableById('table-1');
     ...
     * @param {FieldId} fieldId The attachment field id.
     * @param {IBaseAttachment[]} attachments Attachment descriptors to store. Pass an empty array to clear the field.
     * @returns {boolean} `true` if the undoable command succeeded, `false` otherwise.
     * @example Store a public URL
     * ```ts
     * const record = univerAPI.getActiveBase()
     *   .getTableById('table-1')
     *   .getRecordById('record-1');
     ...
     * console.log(success);
     * ```
     * @example Store the file id returned by a gRPC upload in a headless process
     * ```ts
     * const fBase = univerAPI.getActiveBase();
     ...
     * console.log(success);
     * ```
     * @example Encode a local file in a Node/headless process and store it directly
     * ```ts
     ...
     * @param {FieldId} fieldId The attachment field id.
     * @returns {IBaseAttachment[]} Attachment descriptors in display order.
     * @throws {Error} If `fieldId` is missing or is not an Attachment field.
     * @example Read attachments
     * ```ts
     * const record = univerAPI.getActiveBase()
     *   .getTableById('table-1')
     *   ?.getRecordById('record-1');
     ...
     * @param {FieldId} fieldId The attachment field id.
     * @param {readonly IBaseAttachment[]} attachments The attachment descriptors to delete.
     * @returns {boolean} `true` if the undoable command succeeded or no matching attachment exists.
     * @throws {Error} If `fieldId` is missing or is not an Attachment field.
     * @example Delete selected attachments
     * ```ts
     * const record = univerAPI.getActiveBase()
     *   .getTableById('table-1')
     *   ?.getRecordById('record-1');
     ...
     * @param {Record<string, BaseCellValue>} values Field-id keyed values to write. Only the provided fields are
     * patched; omitted fields keep their existing values.
     * @param {BaseFieldKeyEnum} fieldKey How to interpret the keys in `values`. Defaults to `BaseFieldKeyEnum.Id`.
     ...
     *
     * @param {Partial<Omit<IRecordSnapshot, 'id'>>} [record] Optional record snapshot to override the source record.
     * @returns {FBaseTableRecord} The new record.
     * @throws {BaseHierarchyError} If the duplicated Parent edge is invalid.
     * @example
     * ```ts
     * const fBase = univerAPI.getActiveBase();
     * const fBaseTable = fBase.getTableById('table-1');
     ...
     * @param {string} orderKey The new order key.
     * @returns {boolean} `true` if the command succeeded, `false` otherwise.
     * @example
     * ```ts
     * const fBase = univerAPI.getActiveBase();
     ...
     * @param {FieldId} fieldId The RecordLink field id.
     * @returns {string[]} Ordered target Record IDs.
     * @throws {Error} If `fieldId` is missing or is not a RecordLink field.
     * @example Read linked records
     * ```ts
     * const fBase = univerAPI.getActiveBase();
     ...
     * @param {FieldId} fieldId The RecordLink field id.
     * @param {readonly string[]} recordIds Target Record IDs in display order. Pass `[]` to clear the field.
     * @returns {boolean} Whether the undoable command succeeded.
     * @throws {Error} If the field is not a RecordLink, its config is invalid, a target is missing, or a single-link field receives multiple IDs.
     * @example Replace or clear linked records
     * ```ts
     * const task = univerAPI.getActiveBase()
     *   .getTableById('tasks-table')
     ...
     * @returns {boolean} Whether the undoable command succeeded.
     * @throws {Error} If the field or target record is invalid.
     * @example
     * ```ts
     * const task = univerAPI.getActiveBase()
     *   .getTableById('tasks-table')
     ...
     * `null`.
     *
     * @param {FieldId} fieldId The id returned by `table.getHierarchyFieldId()`.
     * The deterministic virtual id is valid before the first hierarchy write.
     * @returns {FBaseTableRecord | null} The effective parent, or `null` when this
     * record is a root or its stored Parent edge is invalid.
     * @throws {Error} If `fieldId` is neither the table's materialized Parent field
     ...
     *
     * @param {FieldId} fieldId The id returned by `table.getHierarchyFieldId()`.
     * @returns {FBaseTableRecord[]} Direct children ordered by the table's manual
     * record order. The returned array is a snapshot and can be modified safely.
     * @throws {Error} If `fieldId` is not the table's actual or virtual Parent field.
     * @example
     * ```ts
     * const fieldId = table.getHierarchyFieldId();
     ...
     *
     * @param {FieldId} fieldId The id returned by `table.getHierarchyFieldId()`.
     * @returns {FBaseTableRecord[]} Ancestors from the direct parent to the root.
     * @throws {Error} If `fieldId` is not the table's actual or virtual Parent field.
     * @example
     * ```ts
     * const fieldId = table.getHierarchyFieldId();
     ...
     *
     * @param {FieldId} fieldId The id returned by `table.getHierarchyFieldId()`.
     * @returns {FBaseTableRecord[]} All descendants in pre-order depth-first order.
     * @throws {Error} If `fieldId` is not the table's actual or virtual Parent field.
     * @example
     * ```ts
     * const fieldId = table.getHierarchyFieldId();
     ...
     *
     * const subtreeRecordIds = epic
     *   .getDescendants(fieldId)
     ...
     *
     * @param {FieldId} fieldId The id returned by `table.getHierarchyFieldId()`.
     * @param {string | null} parentRecordId The new direct parent's record id, or
     * `null` to move this record to the root level.
     * @param {string} [orderKey] Optional table-level manual order key to update in
     * the same undoable command.
     * @returns {boolean} `true` when the atomic command succeeds. Moving to the same
     * parent and order is a successful no-op.
     * @throws {BaseHierarchyError} With a code from
     * `univerAPI.Enum.BaseHierarchyErrorCode` for an invalid field, missing record,
     * self-parent, cycle, or depth overflow.
     * @example Move a subtree and handle a depth failure
     * ```ts
     ...
     *
     * @param {FieldId} fieldId The id returned by `table.getHierarchyFieldId()`.
     * @param {Record<FieldId, BaseCellValue>} values Initial field values. Do not
     * include the Parent field; this method writes it from the current record.
     * @param {BaseFieldKeyEnum} [fieldKey] Whether `values` keys are field ids or
     * field display names. Defaults to `BaseFieldKeyEnum.Id`.
     * @param {Partial<Omit<IRecordSnapshot, 'id'>>} [record] Optional record metadata.
     * @returns {FBaseTableRecord} A Facade object for the newly created child,
     ...
     * or depth overflow.
     * @throws {Error} If record validation or the atomic command fails.
     * @example Create the first hierarchy edge using field ids
     * ```ts
     * const base = univerAPI.getActiveBase();
     * if (!base) throw new Error('No active Base.');
     *
     ...
     * ```
     * @example Create a child using field display names
     * ```ts
    ...
    private _getRecord;
    private _getRecordLinkField;
    private _getAttachmentField;
}
