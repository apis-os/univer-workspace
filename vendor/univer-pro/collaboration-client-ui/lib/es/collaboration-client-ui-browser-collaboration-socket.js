...
let ur = class extends collaborationSocketService {
  constructor(var_core_value_sigB680, var_core_value_sig1F64, var_core_value_sigDD1C, var_core_value_sig2C39, var_core_value_sigB7D11) {
    super(var_core_value_sigB680, var_core_value_sig1F64, var_core_value_sigDD1C, var_core_value_sig2C39, var_core_value_sigB7D11);
  }
  async createSocket(var_core_value_sig64F01) {
    let var_core_value_sig85B1 = await this._getSessionTicket(),
      var_core_value_sig3141 = this._createSocketURL(var_core_value_sig64F01, var_core_value_sig85B1);
    return this._doCreateSocket(var_core_value_sig3141);
  }
  _createSocketURL(var_core_value_sig2162, var_core_value_sig2EAD) {
    let url4 = typeof window > 'u' ? new URL(var_core_value_sig2162) : new URL(var_core_value_sig2162, window.location["origin"]);
    url4.searchParams["set"]("sessionTicket", var_core_value_sig2EAD);
    let url5 = typeof window > 'u' ? new URLSearchParams() : new URLSearchParams(window.location["search"]);
    return url5.get("runId") && url4.searchParams["set"]("runId", url5.get("runId")), url4.toString();
  }
  async _getSessionTicket() {
    var targetObj292;
    let targetObj293 = this._configService["getConfig"](cOLLABORATION_CLIENT_PLUGIN_CONFIG_KEY),
      var_core_value_sig7DF12 = (targetObj293 == null ? undefined : targetObj293.wsSessionTicketUrl) ?? dEFAULT_WS_SESSION_TICKET_URL;
    return ((targetObj292 = (await this._httpService["get"](var_core_value_sig7DF12, {
      'headers': {
        'Content-Type': 'application/json'
      }
    })).body) == null ? undefined : targetObj292.ticket) ?? '';
  }
};
export { ur as BrowserCollaborationSocketService };
