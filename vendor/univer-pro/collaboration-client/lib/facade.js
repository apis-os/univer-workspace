import { CollaborationController as var_core_value_sig4805, CollaborationStatus as var_core_value_sigE67E, DataLoaderService as var_core_value_sig2902, MemberService as var_core_value_sig9989 } from "@univerjs-pro/collaboration-client";
import { FBase as var_core_value_sig698E, FEnum as var_core_value_sig2809, FEventName as var_core_value_sig2DAB, FUniver as var_core_value_sig877E } from "@univerjs/core/facade";
import { DisposableCollection as var_core_value_sig20C8, IUniverInstanceService as var_core_value_sigE9A7, Inject as var_core_value_sigBECE, Injector as var_core_value_sig1B22, toDisposable as var_core_value_sig7F72 } from "@univerjs/core";
import { SnapshotService as var_core_value_sig7B2A } from "@univerjs-pro/collaboration";
import { switchMap as var_core_value_sig06CD } from "rxjs";
var h = class extends var_core_value_sig2809 {
  get CollaborationStatus() {
    return var_core_value_sigE67E;
  }
};
var_core_value_sig2809.extend(h);
var g = class extends var_core_value_sig2DAB {
  get CollaborationStatusChanged() {
    return "CollaborationStatusChanged";
  }
};
var_core_value_sig2DAB.extend(g);
function _(var_core_value_sigFDEA, var_core_value_sig86D0) {
...
function v(var_core_value_sig4CD2, var_core_value_sig48CA, var_core_value_sig50AF, var_core_value_sigA942) {
...
let y = class extends var_core_value_sig698E {
  ...
  subscribeCollaborators(var_core_value_sigE154, var_core_value_sig4632) {
    return var_core_value_sig7F72(this._injector["get"](var_core_value_sig9989).waitForRoom$(var_core_value_sigE154).pipe(var_core_value_sig06CD(var_core_value_sig27E5 => var_core_value_sig27E5.members$)).subscribe(var_core_value_sig8061 => var_core_value_sig4632(Array.from(var_core_value_sig8061.values()))));
  }
  getCollaborationStatus(var_core_value_sig12F2) {
    let var_core_value_sig2259 = this._resolveUnitId(var_core_value_sig12F2);
    if (!var_core_value_sig2259) return var_core_value_sigE67E.NOT_COLLAB;
    let var_core_value_sig9E2F = this._collaborationController["getCollabEntity"](var_core_value_sig2259);
    return (var_core_value_sig9E2F == null ? undefined : var_core_value_sig9E2F.state["status"]) ?? var_core_value_sigE67E.NOT_COLLAB;
  }
  flush(var_core_value_sigD082, var_core_value_sigDBB7 = {}) {
    ...
    if (!var_core_value_sigF4B9) return Promise.reject(Error('[FCollaboration]: Unit "' + var_core_value_sigD0A8 + '" is not in collaboration mode.'));
    let var_core_value_sig5CEE = var_core_value_sigF4B9.state["status"];
    if (var_core_value_sig5CEE === var_core_value_sigE67E.SYNCED) return Promise.resolve();
    let var_core_value_sigE92A = b(var_core_value_sigD0A8, var_core_value_sig5CEE);
    ...
    return new Promise((var_core_value_sig4D4C, var_core_value_sigC9E0) => {
      ...
      var_core_value_sigFBFA = var_core_value_sigF4B9.status$["subscribe"](var_core_value_sig2AD0 => {
        ...
        if (var_core_value_sig2AD0 === var_core_value_sigE67E.SYNCED) {
          var_core_value_sig1BBD(), var_core_value_sig4D4C();
          return;
        }
        let var_core_value_sig3EEE = b(var_core_value_sigD0A8, var_core_value_sig2AD0);
...
y = v([_(1, var_core_value_sigBECE(var_core_value_sig1B22)), _(2, var_core_value_sigE9A7), _(3, var_core_value_sigBECE(var_core_value_sig7B2A)), _(4, var_core_value_sigBECE(var_core_value_sig4805))], y);
function b(var_core_value_sig5B67, var_core_value_sig1758) {
  return var_core_value_sig1758 === var_core_value_sigE67E.CONFLICT || var_core_value_sig1758 === var_core_value_sigE67E.NOT_COLLAB ? Error('[FCollaboration]: Cannot flush unit "' + var_core_value_sig5B67 + "\x22\x20while\x20collaboration\x20status\x20is\x20" + var_core_value_sig1758 + ".") : null;
}
var x = class extends var_core_value_sig877E {
  _initialize(var_core_value_sigEFD4) {
    let var_core_value_sig861B = var_core_value_sigEFD4.get(var_core_value_sig4805);
    this.disposeWithMe(this.registerEventHandler(this.Event["CollaborationStatusChanged"], () => {
      let var_core_value_sigF704 = new var_core_value_sig20C8(),
        var_core_value_sig2BCF = var_core_value_sig861B.entityInit$["subscribe"](var_core_value_sigBC46 => {
          ...
          var_core_value_sigF704.add(var_core_value_sig7F72(var_core_value_sig3D7D));
        });
      return var_core_value_sigF704.add(var_core_value_sig7F72(var_core_value_sig2BCF)), var_core_value_sigF704;
    }));
  }
  getCollaboration() {
  ...
  loadServerUnit(var_core_value_sig5237, var_core_value_sigBB00, var_core_value_sig7E54) {
    return this._injector["get"](var_core_value_sig2902).loadUnit(var_core_value_sig5237, var_core_value_sigBB00, var_core_value_sig7E54);
  }
  loadServerUnitOfRevision(var_core_value_sig9A8D, var_core_value_sigC259, var_core_value_sig9C9F) {
    return this._injector["get"](var_core_value_sig2902).loadUnitOfRevision(var_core_value_sig9A8D, var_core_value_sigC259, var_core_value_sig9C9F);
  }
};
var_core_value_sig877E.extend(x);
