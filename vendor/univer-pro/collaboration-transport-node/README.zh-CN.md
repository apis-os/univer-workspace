...
Collaboration、History、Comment 和 Worktree Endpoint 可以注册在同一个
Transport 上，共享同一组 HTTP 认证 middleware。Transport 执行到 Endpoint Router 时匹配
路由，未命中则继续后面的注册项；所有已注册 Endpoint 都未处理的 HTTP 请求由 Transport 返回纯文本
...
将认证、观测 middleware 注册在需要处理的 Endpoint 前。`use()`、`useUpgrade()` 和
`register()` 按注册顺序执行。`await next()` 等待后续 middleware 和 handler 完成；
middleware 也可以自行结束响应并省略 `next()`。

HTTP 和 upgrade middleware 可以通过 `ctx.params` 读取解码后的路径参数，通过
`ctx.route?.path` 读取完整路由模板。匹配前 params 为空，route 未定义。
请求应使用各 Endpoint 文档列出的精确协议路径，包括大小写和斜杠。

统计 HTTP 耗时时，在响应结束时读取路由元数据：
...
`recordHttpDuration` 是应用提供的回调。路由匹配前被拒绝的请求和未知路径没有 route。
要记录中途断开的响应，可监听 `close` 并与 `finish` 去重。`await next()` 后的 handler
完成时机可能早于响应完成。
