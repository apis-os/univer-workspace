...
重试同一个 draft submission 时保持 `unitID/type/sid/reqId`；`baseRev` 只能越过排在它
之前的 confirmed changesets，不能越过自身 confirmed revision。Service 在读取并验证
`(baseRev, draftHead]` history 后、执行 OT 和 `applyChangeset` 前检查重复提交；Database
Adapter 只用 revision CAS 串行化多实例竞态，失败方 reload 后从新 history 命中 submission。

## 常用 API

- `createWorktree()` / `getWorktree()`：创建和读取 Worktree。
- `addUnit()`：把已有 trunk Unit 加入 Worktree。
- `createUnitFromSnapshot()` / `createUnitFromData()`：创建 Worktree 本地 Unit。
- `getUnitLoadData()` / `getChangesets()`：读取 draft 协同数据。
