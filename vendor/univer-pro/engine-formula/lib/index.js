...
let Bt = class extends a {
  constructor(e, t, n) {
    super(), this.disposeWithMe(e.registerPluginResource({
      pluginName: St,
...
let Yn = class {
  ...
  assess(e, t) {
    if ((t == null ? void 0 : t.status) !== ge.SUCCESS) return {
      eligible: !1,
      reason: `not-successful`
    };
    let n = e.startsWith(`=`) ? e.slice(1) : e;
    return (this._lexerTreeBuilder.sequenceNodesBuilder(n) ?? []).some(e => typeof e != `string` && e.nodeType === Je.FUNCTION && Jn.has(e.token.toUpperCase())) ? {
      eligible: !1,
      reason: `volatile`
    } : {
      eligible: !0
...
function hr(e) {
  let {
    ...
    i = n.unitId === t;
  if (r.kind === `sheet-range`) return i ? Ye(r.sheetName, r.range) : Xe(n.formulaQualifier, r.sheetName, r.range);
  let a = r.endColumnName && r.endColumnName !== r.columnName ? `[[${_r(r.columnName)}]:[${_r(r.endColumnName)}]]` : `[${_r(r.columnName)}]`,
    o = `${gr(r.tableName)}${a}`;
...
function Er(e) {
  ...
  for (let n of e) {
    ...
    let e = ``;
    n.nodeType === Je.REFERENCE ? e = He(n.token).unitId : n.nodeType === Je.TABLE && (e = Ze(n.token).unitQualifier);
    let r = e.trim();
    r && t.set(M(r), r);
  }
  return Array.from(t.values());
...
export { Ct as EXTERNAL_REFERENCE_SCHEMA_VERSION, Et as ExternalCacheReferenceDataProvider, qn as ExternalFormulaUnitReferenceResolver, Nt as ExternalReferenceDataProvidersController, I as ExternalReferenceModel, B as ExternalReferenceOverlayModel, or as ExternalReferencePrefetchPlanner, lr as ExternalReferencePrefetchService, zt as ExternalReferenceRenameController, Bt as ExternalReferenceResourceController, Vt as FORMULA_REFERENCE_DATA_RPC_CHANNEL, Yn as FormulaCacheEligibilityService, ue as FormulaCalculationTriggerService, de as FormulaDataModel, bt as FormulaLastValuePersistenceService, jt as FormulaReferenceDataProviderRegistry, Ht as FormulaReferenceDataRpcController, mr as FormulaReferenceType, ot as HOST_EXTERNAL_REFERENCE_SCHEMA_VERSION, N as HostExternalReferenceModel, ve as IActiveDirtyManagerService, At as IFormulaReferenceDataProviderRegistry, z as IFormulaReferenceDataService, De as ISuperTableService, $ as MainFormulaReferenceDataService, xt as PersistFormulaLastValuesCommand, pt as RemoveHostExternalReferenceCommand, Pe as RemoveSuperTableMutation, St as SHEET_EXTERNAL_DATA_PLUGIN, Fe as SetFormulaCalculationResultMutation, Ie as SetFormulaCalculationStartMutation, Re as SetFormulaDataMutation, ze as SetSuperTableMutation, Be as SetTriggerFormulaCalculationStartMutation, at as UNIVER_EXTERNAL_REFERENCE_PLUGIN, tr as UniverProFormulaEnginePlugin, dt as UpsertHostExternalReferenceCommand, ft as UpsertHostExternalReferencesCommand, hr as buildFormulaReference, gt as buildUpsertHostExternalReferenceMutations, Er as collectFormulaExternalReferenceQualifiers, wt as createEmptyExternalReferenceResource, F as createExternalSyntheticUnitId, Mt as createUnavailableReferenceDataResponse, br as getFormulaResultCell, xr as getScalarFormulaResultCell, M as normalizeHostExternalReferenceQualifier, qe as refactorFormulaUnitQualifier, yr as resolveFormulaResultPresentation, Dr as resolveHostFormulaExternalReferences, Cr as toFormulaLastValue };
