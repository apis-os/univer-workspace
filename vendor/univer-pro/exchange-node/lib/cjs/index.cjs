...
var import_collaboration = require("@univerjs-pro/collaboration"),
  import_core = require("@univerjs/core"),
...
// src/types.ts
var ExchangeFormat = /* @__PURE__ */ ((ExchangeFormat2) => (
    (ExchangeFormat2.XLS = "xls"),
    (ExchangeFormat2.XLSX = "xlsx"),
    (ExchangeFormat2.CSV = "csv"),
    (ExchangeFormat2.TSV = "tsv"),
    (ExchangeFormat2.DOC = "doc"),
    (ExchangeFormat2.DOCX = "docx"),
    (ExchangeFormat2.PPT = "ppt"),
    (ExchangeFormat2.PPTX = "pptx"),
    ExchangeFormat2
  ))(ExchangeFormat || {}),
  FormulaCalculationMode = /* @__PURE__ */ ((FormulaCalculationMode2) => (
    (FormulaCalculationMode2.FORCED = "forced"),
    (FormulaCalculationMode2.WHEN_EMPTY = "when_empty"),
    (FormulaCalculationMode2.NO = "no"),
    FormulaCalculationMode2
  ))(FormulaCalculationMode || {}),
  CsvTypeInferenceMode = /* @__PURE__ */ ((CsvTypeInferenceMode2) => (
    (CsvTypeInferenceMode2.OFF = "off"),
    (CsvTypeInferenceMode2.SAFE = "safe"),
    (CsvTypeInferenceMode2.AGGRESSIVE = "aggressive"),
    CsvTypeInferenceMode2
  ))(CsvTypeInferenceMode || {}),
  CsvQuoteStyle = /* @__PURE__ */ ((CsvQuoteStyle2) => (
    (CsvQuoteStyle2.MINIMAL = "minimal"),
    (CsvQuoteStyle2.ALWAYS = "always"),
    CsvQuoteStyle2
  ))(CsvQuoteStyle || {}),
  CsvLineEnding = /* @__PURE__ */ ((CsvLineEnding2) => (
    (CsvLineEnding2.LF = "lf"),
    (CsvLineEnding2.CRLF = "cr_lf"),
    CsvLineEnding2
  ))(CsvLineEnding || {}),
  CsvValueMode = /* @__PURE__ */ ((CsvValueMode2) => (
    (CsvValueMode2.DISPLAY = "display"),
    (CsvValueMode2.RAW = "raw"),
    CsvValueMode2
  ))(CsvValueMode || {}),
  CsvFormulaMode = /* @__PURE__ */ ((CsvFormulaMode2) => (
    (CsvFormulaMode2.CACHED = "cached"),
    (CsvFormulaMode2.EXPRESSION = "expression"),
    CsvFormulaMode2
  ))(CsvFormulaMode || {}),
  BaseImportMode = /* @__PURE__ */ ((BaseImportMode2) => (
    (BaseImportMode2.TABLES = "tables"),
    (BaseImportMode2.SHEETS = "sheets"),
    (BaseImportMode2.HYBRID = "hybrid"),
    (BaseImportMode2.AUTO = "auto"),
    BaseImportMode2
  ))(BaseImportMode || {}),
  BaseTypeInference = /* @__PURE__ */ ((BaseTypeInference2) => (
    (BaseTypeInference2.NONE = "none"),
    (BaseTypeInference2.SAFE = "safe"),
    (BaseTypeInference2.AGGRESSIVE = "aggressive"),
    BaseTypeInference2
  ))(BaseTypeInference || {}),
  BaseFormulaPolicy = /* @__PURE__ */ ((BaseFormulaPolicy2) => (
    (BaseFormulaPolicy2.VALUES = "values"),
    (BaseFormulaPolicy2.TEXT = "text"),
    (BaseFormulaPolicy2.FAIL = "fail"),
    (BaseFormulaPolicy2.CONVERT_THEN_VALUES = "convertThenValues"),
    BaseFormulaPolicy2
  ))(BaseFormulaPolicy || {}),
  BaseImagePolicy = /* @__PURE__ */ ((BaseImagePolicy2) => (
    (BaseImagePolicy2.SAFE = "safe"),
    (BaseImagePolicy2.CELL_ONLY = "cell-only"),
    (BaseImagePolicy2.MEDIA_TABLE = "media-table"),
    (BaseImagePolicy2.NONE = "none"),
    BaseImagePolicy2
  ))(BaseImagePolicy || {}),
  BaseHeaderPolicy = /* @__PURE__ */ ((BaseHeaderPolicy2) => (
    (BaseHeaderPolicy2.FIRST_ROW = "firstRow"),
    (BaseHeaderPolicy2.NONE = "none"),
    BaseHeaderPolicy2
  ))(BaseHeaderPolicy || {}),
  BaseExportMode = /* @__PURE__ */ ((BaseExportMode2) => (
    (BaseExportMode2.TABLES = "tables"),
    (BaseExportMode2.RANGES = "ranges"),
    BaseExportMode2
  ))(BaseExportMode || {}),
  BaseAttachmentPolicy = /* @__PURE__ */ ((BaseAttachmentPolicy2) => (
    (BaseAttachmentPolicy2.HYBRID = "hybrid"),
    (BaseAttachmentPolicy2.MANIFEST = "manifest"),
    (BaseAttachmentPolicy2.LINKS = "links"),
    (BaseAttachmentPolicy2.NONE = "none"),
    BaseAttachmentPolicy2
  ))(BaseAttachmentPolicy || {}),
  DocxCompatibilityMode = /* @__PURE__ */ ((DocxCompatibilityMode2) => (
    (DocxCompatibilityMode2.TRADITIONAL = "traditional"),
    (DocxCompatibilityMode2.MODERN = "modern"),
    DocxCompatibilityMode2
  ))(DocxCompatibilityMode || {}),
  ExchangeErrorCode = /* @__PURE__ */ ((ExchangeErrorCode2) => (
    (ExchangeErrorCode2.INVALID_ARGUMENT = "INVALID_ARGUMENT"),
    (ExchangeErrorCode2.UNSUPPORTED_FORMAT = "UNSUPPORTED_FORMAT"),
    (ExchangeErrorCode2.INVALID_FILE = "INVALID_FILE"),
    (ExchangeErrorCode2.INCOMPLETE_SNAPSHOT = "INCOMPLETE_SNAPSHOT"),
    (ExchangeErrorCode2.IO_ERROR = "IO_ERROR"),
    (ExchangeErrorCode2.NATIVE_LOAD_FAILED = "NATIVE_LOAD_FAILED"),
    (ExchangeErrorCode2.CONVERSION_FAILED = "CONVERSION_FAILED"),
    ExchangeErrorCode2
  ))(ExchangeErrorCode || {}),
...
async function loadNativeBinding() {
  ...
  try {
    return (
      (nativeBinding = nodeRequire("@univerjs-pro/exchange-node-binding")),
      (nativeBindingSource = "@univerjs-pro/exchange-node-binding"),
      nativeBinding
    );
  } catch (error) {
  ...
  return {
    ...
    service: {
      async saveSheetBlock(_context, { block }) {
        return block
          ? (sheetBlocks.set(block.id, block),
            {
              blockID: block.id,
          ...
          : {
              blockID: "",
              error: {
                code: import_protocol.ErrorCode.UNDEFINED,
...
function bufferView(bytes) {
  return import_node_buffer.Buffer.isBuffer(bytes)
    ? bytes
...
function toNativeSnapshot(value) {
  ...
  if (sourceWorkbook) {
    ...
    (sourceSheets &&
      (workbook.sheets = Object.fromEntries(
  ...
  for (let entry of metadata) {
    ...
    if (entry.kind === "worksheet") {
      ...
      if (
        ...
        throw new TypeError(
          `unsupported native snapshot metadata kind ${entry.kind}`,
        );
      owner = snapshot[entry.kind];
    }
    if (!owner)
      throw new TypeError(
        `native snapshot metadata owner ${entry.kind}/${entry.ownerId ?? ""} is missing`,
      );
...
function fromNativeSnapshot(value) {
  ...
  return (
    restoreMetadata(snapshot, value.metadata),
    {
      snapshot,
      sheetBlocks: value.sheetBlocks.map((block) => ({
        id: block.id,
...
function normalizeWorkbookSheetOrder(data) {
  let existingOrder = Array.isArray(data.sheetOrder)
      ? data.sheetOrder.filter((sheetId) => typeof sheetId == "string")
      : [],
    ordered = new Set(existingOrder),
    missingSheetIds = Object.keys(data.sheets).filter(
      (sheetId) => !ordered.has(sheetId),
    );
  return missingSheetIds.length === 0
    ? data
    : { ...data, sheetOrder: [...existingOrder, ...missingSheetIds] };
}
function exchangeError(error, fallback) {
  ...
  return nativeCode && Object.values(ExchangeErrorCode).includes(nativeCode)
    ? new ExchangeError(nativeCode, message.replace(/^\[[A-Z_]+]\s*/, ""), {
        cause: error,
      })
    : error instanceof TypeError
      ? new ExchangeError("INVALID_ARGUMENT" /* INVALID_ARGUMENT */, message, {
          cause: error,
        })
      : new ExchangeError(fallback, message, { cause: error });
}
function resolveFormat(fileName, format) {
  if (format) return format;
  let inferred =
    FORMAT_BY_EXTENSION[(0, import_node_path.extname)(fileName).toLowerCase()];
  if (!inferred)
    throw new ExchangeError(
  ...
  switch (type) {
    ...
    default:
      throw new ExchangeError(
        ...
        `Unsupported Univer unit type: ${type}`,
      );
  }
}
function nativeImportOptions(options, fileName) {
  return {
    ...
    minSheetRowCount:
      "minSheetRowCount" in options ? options.minSheetRowCount : void 0,
    minSheetColumnCount:
      "minSheetColumnCount" in options ? options.minSheetColumnCount : void 0,
    compatibilityMode:
      "compatibilityMode" in options ? options.compatibilityMode : void 0,
    csv: "csv" in options ? options.csv : void 0,
    base:
      options.type === import_core.UniverInstanceType.UNIVER_BASE
        ? {
            mode: options.mode,
        ...
        : void 0,
    formulaCalculation:
      "formulaCalculation" in options ? options.formulaCalculation : void 0,
  };
}
...
async function snapshotToMemory(snapshot, type) {
  try {
    switch (type) {
      case import_core.UniverInstanceType.UNIVER_SHEET:
        return await (0, import_collaboration.transformSnapshotToWorkbookData)(
          snapshot.snapshot,
          snapshot.sheetBlocks,
        );
      case import_core.UniverInstanceType.UNIVER_BASE:
        return await (0, import_collaboration.transformSnapshotToBaseData)(
          snapshot.snapshot,
          snapshot.sheetBlocks,
        );
      case import_core.UniverInstanceType.UNIVER_DOC:
        return (0, import_collaboration.transformSnapshotToDocumentData)(
          snapshot.snapshot,
        );
      case import_core.UniverInstanceType.UNIVER_SLIDE:
        return (0, import_collaboration.transformSnapshotToSlideData)(
          snapshot.snapshot,
        );
      default:
...
async function memoryToSnapshot(data, options) {
  try {
    ...
    if (!unitId)
      throw new ExchangeError(
        "INVALID_ARGUMENT" /* INVALID_ARGUMENT */,
        "Export data must have a non-empty id",
      );
    let rev = data.rev ?? 0,
    ...
    switch (options.type) {
      case import_core.UniverInstanceType.UNIVER_SHEET: {
        let workbook = normalizeWorkbookSheetOrder(data),
          { snapshot } = await (0,
          import_collaboration.transformWorkbookDataToSnapshot)(
            { metadata: void 0 },
            workbook,
            unitId,
            rev,
      ...
      case import_core.UniverInstanceType.UNIVER_BASE: {
        ...
        import_collaboration.transformBaseDataToSnapshot)(
          { metadata: void 0 },
          data,
          unitId,
          rev,
      ...
      case import_core.UniverInstanceType.UNIVER_DOC: {
        let { snapshot } = await (0,
        import_collaboration.transformDocumentDataToSnapshot)(
          { metadata: void 0 },
          data,
          unitId,
          rev,
      ...
      case import_core.UniverInstanceType.UNIVER_SLIDE: {
        let slide = data,
          { snapshot } = await (0,
          import_collaboration.transformSlideDataToSnapshot)(
            slide.resources,
            slide,
            unitId,
            rev,
...
async function importFileToSnapshot(path, options) {
  let fileName = options.fileName ?? (0, import_node_path.basename)(path),
    input;
  try {
...
async function exportToFile(data, path, options) {
  await exportSnapshotToFile(
    await memoryToSnapshot(data, options),
    path,
    options,
  );
}
