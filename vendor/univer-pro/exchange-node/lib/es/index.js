...
function bufferView(bytes) {
  return NodeBuffer.isBuffer(bytes)
    ? bytes
    : NodeBuffer.from(bytes.buffer, bytes.byteOffset, bytes.byteLength);
}
...
function nativeImportOptions(options, fileName) {
  return {
    ...
    base:
      options.type === UniverInstanceType.UNIVER_BASE
        ? {
            mode: options.mode,
    ...
    switch (options.type) {
      case UniverInstanceType.UNIVER_SHEET: {
        let workbook = normalizeWorkbookSheetOrder(data),
          { snapshot } = await transformWorkbookDataToSnapshot(
            { metadata: void 0 },
            workbook,
            unitId,
            rev,
      ...
      case UniverInstanceType.UNIVER_SLIDE: {
        let slide = data,
          { snapshot } = await transformSlideDataToSnapshot(
            slide.resources,
            slide,
            unitId,
            rev,
