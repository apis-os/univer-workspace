...
export interface ISetPivotShowDataAsMutationParams {
    unitId: string;
    subUnitId: string;
    pivotTableId: string;
    fieldId: string;
...
export interface ISetPivotValueFilterMutationParams {
    ...
    /** Evaluation index. Legacy mutations without this field keep the current index or append a new rule. */
    index?: number;
}
...
export interface IPivotRenderCache {
    ...
    projection?: IPivotSheetProjection;
}
export interface IPivotSheetProjection {
    startColumn: number;
    endColumn: number;
    direction: 'ltr' | 'rtl';
}
