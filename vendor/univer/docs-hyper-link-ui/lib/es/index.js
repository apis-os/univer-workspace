import { DocCanvasPopManagerService, DocEventManagerService, EMPTY_PARAGRAPH_MENU_ID, FLOAT_TOOLBAR_MENU_POSITION, INSERT_BELLOW_MENU_ID, UniverDocsUIPlugin, whenDocAndEditorFocused } from "@univerjs/docs-ui";
...
import { BuildTextUtils, CommandType, CustomRangeType, DOCS_NORMAL_EDITOR_UNIT_ID_KEY, DependentOn, Disposable, DocumentDataModel, ICommandService, IConfigService, IPermissionService, IUniverInstanceService, Inject, Injector, LocaleService, Plugin, Tools, UniverInstanceType, generateRandomId, getBodySlice, isSafeUrl, merge } from "@univerjs/core";
import { DOC_INTERCEPTOR_POINT, DocInterceptorService, DocSelectionManagerService, DocSkeletonManagerService, SetTextSelectionsOperation, UniverDocsPlugin, addCustomRangeBySelectionFactory, canEditDocumentTargets, deleteCustomRangeFactory, getDocumentEntityParentPermissionObjectIds, getDocumentEntityPermissionObjectId, getDocumentPermissionValue, replaceSelectionFactory } from "@univerjs/docs";
...
const DocLinkPopup = () => {
	...
	const doc = univerInstanceService.getUnit(unitId, UniverInstanceType.UNIVER_DOC);
	const body = doc instanceof DocumentDataModel ? (_doc$getSelfOrHeaderF = doc.getSelfOrHeaderFooterModel(segmentId)) === null || _doc$getSelfOrHeaderF === void 0 ? void 0 : _doc$getSelfOrHeaderF.getBody() : void 0;
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [/* @__PURE__ */ jsxs("div", {
			...
			children: [/* @__PURE__ */ jsx(Button, {
				...
				variant: "text",
				size: "small",
				"aria-label": localeService.t("docs-hyper-link-ui.info.copy"),
				className: "univer-ml-2 !univer-flex univer-size-6 !univer-rounded !univer-border-0 !univer-bg-transparent !univer-p-0 !univer-text-base !univer-font-normal hover:!univer-bg-transparent active:!univer-bg-transparent disabled:!univer-pointer-events-auto disabled:!univer-opacity-40",
				disabled: !canCopy,
				onClick: () => {
			...
			}), canEdit && /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsx("div", {
...
function isSameLinkInfo(current, next) {
	return (current === null || current === void 0 ? void 0 : current.linkId) === next.linkId && current.unitId === next.unitId && (current.segmentId ?? "") === (next.segmentId ?? "") && current.segmentPage === next.segmentPage && current.startIndex === next.startIndex && current.endIndex === next.endIndex;
}
let DocHyperLinkPopupService = class DocHyperLinkPopupService extends Disposable {
	constructor(_docCanvasPopupManagerService, _textSelectionManagerService, _univerInstanceService, _permissionService, _renderManagerService) {
		super();
		...
		_defineProperty(this, "_editPopupUnitId", null);
		_defineProperty(this, "_infoPopup", null);
		_defineProperty(this, "_infoPopupPinned", false);
		_defineProperty(this, "_infoPopupHideTimer", null);
		...
		this.disposeWithMe(this._permissionService.permissionPointUpdate$.subscribe(() => {
			...
			if (editing && !this.canEditLink(editing.unitId, editing)) this.hideEditPopup();
			const showing = this.showing;
			if (showing) this._showingLink$.next({ ...showing });
		}));
		this.disposeWithMe(this._renderManagerService.disposed$.subscribe((unitId) => {
			var _this$showing;
			if (this._editPopupUnitId === unitId) this.hideEditPopup();
			if (((_this$showing = this.showing) === null || _this$showing === void 0 ? void 0 : _this$showing.unitId) === unitId) this.hideInfoPopup();
		}));
	}
	dispose() {
		this.hideEditPopup();
	...
	showEditPopup(unitId, linkInfo) {
		if (!this.canEditLink(unitId, linkInfo)) return null;
		if (this._editPopup) {
	...
	showInfoPopup(info, options) {
		this.cancelScheduledHideInfoPopup();
		if (this._infoPopupSuppressed) return;
		const { linkId, unitId, segmentId, segmentPage, startIndex, endIndex } = info;
		if (isSameLinkInfo(this.showing, info)) {
			if (options === null || options === void 0 ? void 0 : options.pinned) this._infoPopupPinned = true;
			return;
		}
		if (this._infoPopup) {
		...
		if (!(this._univerInstanceService.getUnit(unitId, UniverInstanceType.UNIVER_DOC) instanceof DocumentDataModel)) return;
		this._infoPopupPinned = (options === null || options === void 0 ? void 0 : options.pinned) ?? false;
	...
	scheduleHideInfoPopup() {
		if (this._infoPopupPinned || !this.showing) return;
		this.cancelScheduledHideInfoPopup();
		this._infoPopupHideTimer = setTimeout(() => {
			this._infoPopupHideTimer = null;
			if (!this._infoPopupPinned) this.hideInfoPopup();
	...
	canEditLink(unitId, linkInfo) {
		const document = this._univerInstanceService.getUnit(unitId, UniverInstanceType.UNIVER_DOC);
		if (!(document instanceof DocumentDataModel)) return false;
		if (!linkInfo) return canEditDocumentTargets(this._permissionService, unitId, []);
		const segmentId = linkInfo.segmentId ?? "";
		return canEditDocumentTargets(this._permissionService, unitId, [...getDocumentEntityParentPermissionObjectIds(document, segmentId, "custom-range", linkInfo.linkId), getDocumentEntityPermissionObjectId(segmentId, "custom-range", linkInfo.linkId)]);
	}
};
		...
		this.disposeWithMe(this._commandService.onCommandExecuted((commandInfo) => {
			if (commandInfo.id === SetTextSelectionsOperation.id) {
				...
				if ((primary === null || primary === void 0 ? void 0 : primary.collapsed) && doc) {
					...
					const customRange = ((showing === null || showing === void 0 ? void 0 : showing.unitId) === unitId && (showing.segmentId ?? "") === segmentId && showing.segmentPage === segmentPage && showing.endIndex === endOffset - 1 ? customRanges === null || customRanges === void 0 ? void 0 : customRanges.find((value) => value.rangeId === showing.linkId && value.rangeType === CustomRangeType.HYPERLINK && value.startIndex <= startOffset && value.endIndex === endOffset - 1) : void 0) ?? (customRanges === null || customRanges === void 0 ? void 0 : customRanges.find((value) => value.rangeType === CustomRangeType.HYPERLINK && value.startIndex <= startOffset && value.endIndex > endOffset - 1));
					if (customRange) {
...
let DocHyperLinkEventRenderController = class DocHyperLinkEventRenderController extends Disposable {
	...
	_hideInfoPopup() {
		if (this._hyperLinkPopupService.infoPopupPinned) return;
		if (this._hyperLinkPopupService.showing) this._hyperLinkPopupService.scheduleHideInfoPopup();
	}
	_initPointerDown() {
	...
	_initClick() {
		this.disposeWithMe(this._docEventManagerService.clickCustomRanges$.subscribe((range) => {
			...
			if (link.rangeType !== CustomRangeType.HYPERLINK) return;
...
DocHyperLinkRenderController = __decorate([__decorateParam(1, Inject(DocInterceptorService)), __decorateParam(2, Inject(DocHyperLinkPopupService))], DocHyperLinkRenderController);
