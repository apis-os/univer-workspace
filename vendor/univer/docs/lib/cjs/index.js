...
function collectCustomBlockViewports(dataModel) {
	...
	for (const customBlock of ((_snapshot$body = snapshot.body) === null || _snapshot$body === void 0 ? void 0 : _snapshot$body.customBlocks) ?? []) {
		...
		const viewport = (0, _univerjs_engine_render.getDocsCustomBlockRenderViewport)(dataModel.getUnitId(), customBlock.blockId, {
...
let DocLayoutExecutorService = _DocLayoutExecutorService = class DocLayoutExecutorService extends _univerjs_core.Disposable {
	constructor(_univerInstanceService, _commandService, _localeService, _logService) {
		...
		_defineProperty(this, "_executorStatus$", new rxjs.BehaviorSubject({
			state: "unregistered",
			executor: null,
			diagnostic: null,
			recoveryUnitId: null
		...
		this.disposeWithMe(this._univerInstanceService.getTypeOfUnitAdded$(_univerjs_core.UniverInstanceType.UNIVER_DOC).subscribe(({ unit }) => this._createSessionIfEligible(unit)));
		this.disposeWithMe(this._univerInstanceService.getTypeOfUnitDisposed$(_univerjs_core.UniverInstanceType.UNIVER_DOC).subscribe((unit) => this._disposeSession(unit.getUnitId())));
	...
	register(executor) {
		...
		for (const unit of this._univerInstanceService.getAllUnitsForType(_univerjs_core.UniverInstanceType.UNIVER_DOC)) this._createSessionIfEligible(unit);
	...
	registerCustomRangePresentationProvider(provider) {
		...
		return (0, _univerjs_core.toDisposable)(() => this._customRangePresentationProviders.delete(provider));
	}
	...
	_enqueueCommittedMutation(params) {
		...
		if (_univerjs_core.JSONX.isNoop(params.actions)) return;
	...
	_isEligible(dataModel) {
		if ((0, _univerjs_core.isInternalEditorID)(dataModel.getUnitId())) return false;
		const flavor = dataModel.getSnapshot().documentStyle.documentFlavor;
		return flavor === _univerjs_core.DocumentFlavor.TRADITIONAL || flavor === _univerjs_core.DocumentFlavor.MODERN;
	}
	_requireExecutor() {
	...
	_createSessionOnNextMacroTask(executor, session, dataModel) {
		return new Promise((resolve, reject) => {
			const cancelTask = (0, _univerjs_core.requestImmediateMacroTask)(() => {
...
let DocSkeletonManagerService = class DocSkeletonManagerService extends _univerjs_core.RxDisposable {
	...
	supportsIncrementalLayout() {
		...
		return this._docLayoutExecutorService.getExecutor() != null && !(0, _univerjs_core.isInternalEditorID)(this._context.unitId) && (documentFlavor === _univerjs_core.DocumentFlavor.TRADITIONAL || documentFlavor === _univerjs_core.DocumentFlavor.MODERN);
	}
	recalculate() {
...
DocSkeletonManagerService = __decorate([
	...
	__decorateParam(3, (0, _univerjs_core.Inject)(DocLayoutExecutorService))
...
const STRUCTURAL_DATA_STREAM_TOKENS = /* @__PURE__ */ new Set([
	_univerjs_core.DataStreamTreeTokenType.PARAGRAPH,
...
function isStructurePreservingTextXEdit(value) {
	...
	return value.every((action) => {
		...
		if (action.t === _univerjs_core.TextXActionType.DELETE) return true;
		if (action.t === _univerjs_core.TextXActionType.INSERT) return isStructurePreservingBody(action.body);
		return action.t === _univerjs_core.TextXActionType.RETAIN && (action.body == null || isStructurePreservingBody(action.body));
	});
}
...
function isStructurePreservingJSONXEdit(actions, expectedPath) {
	...
	cursor.traverse(null, (component) => {
		...
		if (component.et !== _univerjs_core.TextX.id || componentKeys.some((key) => key !== "et" && key !== "e") || !pathsEqual(path, expectedPath)) {
...
function validateDocStructureMutation(documentDataModel, segmentId, actions, undoActions) {
	const editPath = (0, _univerjs_core.getRichTextEditPath)(documentDataModel, segmentId);
...
function transformDocumentTextRanges(actions, textRanges, priority = "right") {
	...
	return textRanges.map((textRange) => {
		const startOffset = _univerjs_core.JSONX.transformPosition(bodyActions, textRange.startOffset, priority);
		const endOffset = _univerjs_core.JSONX.transformPosition(bodyActions, textRange.endOffset, priority);
		return {
			...textRange,
			startOffset,
			endOffset,
...
var DocumentCommentPermission = class {
	constructor(unitId) {
		this.unitId = unitId;
		_defineProperty(this, "type", _univerjs_protocol.UnitObject.Document);
...
var DocumentEntityEditPermission = class {
	constructor(unitId, objectId) {
		...
		_defineProperty(this, "type", _univerjs_protocol.UnitObject.DocumentEntity);
		_defineProperty(this, "subType", _univerjs_protocol.UnitAction.Edit);
		_defineProperty(this, "status", _univerjs_core.PermissionStatus.INIT);
		_defineProperty(this, "id", void 0);
		_defineProperty(this, "value", true);
...
var DocumentParagraphEditPermission = class {
	constructor(unitId, objectId) {
		this.unitId = unitId;
		this.objectId = objectId;
		_defineProperty(this, "type", _univerjs_protocol.UnitObject.DocumentParagraph);
		_defineProperty(this, "subType", _univerjs_protocol.UnitAction.Edit);
		_defineProperty(this, "status", _univerjs_core.PermissionStatus.INIT);
		_defineProperty(this, "id", void 0);
		_defineProperty(this, "value", true);
...
var DocumentSectionEditPermission = class {
	constructor(unitId, objectId) {
		this.unitId = unitId;
		this.objectId = objectId;
		_defineProperty(this, "type", _univerjs_protocol.UnitObject.DocumentSection);
		_defineProperty(this, "subType", _univerjs_protocol.UnitAction.Edit);
		_defineProperty(this, "status", _univerjs_core.PermissionStatus.INIT);
		_defineProperty(this, "id", void 0);
		_defineProperty(this, "value", true);
...
const DOCUMENT_PERMISSION_OBJECT_TYPES = /* @__PURE__ */ new Set([
	_univerjs_protocol.UnitObject.Document,
	_univerjs_protocol.UnitObject.DocumentSection,
	_univerjs_protocol.UnitObject.DocumentParagraph,
	_univerjs_protocol.UnitObject.DocumentEntity
]);
...
function createDocumentPermissionPoint(unitId, objectId, action) {
	if (objectId === unitId) switch (action) {
		case _univerjs_protocol.UnitAction.Edit: return new DocumentEditablePermission(unitId);
		case _univerjs_protocol.UnitAction.Copy: return new DocumentCopyPermission(unitId);
		case _univerjs_protocol.UnitAction.Print: return new DocumentPrintPermission(unitId);
		case _univerjs_protocol.UnitAction.Export: return new DocumentExportPermission(unitId);
		case _univerjs_protocol.UnitAction.Comment: return new DocumentCommentPermission(unitId);
		default: throw new Error(`Unsupported Document permission action: ${action}`);
	}
	if (action !== _univerjs_protocol.UnitAction.Edit) throw new Error(`Document object permissions only support Edit: ${objectId}`);
...
function canEditDocumentTargets(permissionService, unitId, objectIds) {
	if (!getDocumentPermissionValue(permissionService, unitId, unitId, _univerjs_protocol.UnitAction.Edit)) return false;
	for (const objectId of objectIds) if (!getDocumentPermissionValue(permissionService, unitId, objectId, _univerjs_protocol.UnitAction.Edit)) return false;
	return true;
}
...
const SetDocumentPermissionCommand = {
	...
	async handler(accessor, params) {
		...
		if (params.objectId !== params.unitId && params.action !== _univerjs_protocol.UnitAction.Edit) return false;
		if (params.policy && (![
			"all",
			"owner",
			"members"
		].includes(params.policy.edit) || params.policy.strategies.some((strategy) => !DOCUMENT_UNIT_PERMISSION_ACTIONS.includes(strategy.action) || params.objectId !== params.unitId && strategy.action !== _univerjs_protocol.UnitAction.Edit))) return false;
		...
		const service = accessor.get(_univerjs_core.ObjectPermissionService);
...
* limitations under the License.
*/
let DocumentPermissionRuleModel = class DocumentPermissionRuleModel extends _univerjs_core.ObjectPermissionRuleModel {
	constructor(resources) {
		super(resources, "DOC_OBJECT_PERMISSION_PLUGIN", _univerjs_core.UniverInstanceType.UNIVER_DOC, [
			_univerjs_protocol.UnitObject.DocumentSection,
			_univerjs_protocol.UnitObject.DocumentParagraph,
			_univerjs_protocol.UnitObject.DocumentEntity
		]);
...
DocumentPermissionRuleModel = __decorate([__decorateParam(0, _univerjs_core.IResourceManagerService)], DocumentPermissionRuleModel);
...
const SetDocumentPermissionRuleMutation = {
	...
	type: _univerjs_core.CommandType.MUTATION,
	handler: (accessor, params) => {
		if (!params || !accessor.get(_univerjs_core.IUniverInstanceService).getUnit(params.unitId, _univerjs_core.UniverInstanceType.UNIVER_DOC)) return false;
...
function getDocumentPermissionSegmentIndex(documentDataModel, segmentId, resolverIndex = getDocumentPermissionResolverIndex(documentDataModel)) {
	...
	const segmentIndex = {
		...
		entities,
		entityRanges,
		paragraphContentStartOffsets: body == null ? /* @__PURE__ */ new Map() : (0, _univerjs_core.getParagraphContentStartOffsets)(body),
...
function getDocumentEditTargetObjectIdsFromActions(documentDataModel, segmentId, actions) {
	...
	if (textActions) {
		...
		textActions.forEach((action) => {
			if (action.t === _univerjs_core.TextXActionType.INSERT) {
				addRange(offset, offset);
				return;
			}
			if (action.t === _univerjs_core.TextXActionType.DELETE) {
...
function getTextXActions(actions) {
	...
	return edit.et === _univerjs_core.TextX.id && Array.isArray(edit.e) && edit.e.every(isTextXAction) ? edit.e : null;
}
function isTextXAction(value) {
	if (!isRecord$2(value) || typeof value.len !== "number") return false;
	if (value.t === _univerjs_core.TextXActionType.INSERT) return isRecord$2(value.body);
	return value.t === _univerjs_core.TextXActionType.RETAIN || value.t === _univerjs_core.TextXActionType.DELETE;
}
...
function getDocumentParagraphParentPermissionObjectIds(documentDataModel, segmentId, paragraphId) {
	...
	return getSectionPermissionObjectIds(documentDataModel, segmentId, {
		startOffset: segmentIndex.paragraphContentStartOffsets.get(paragraph.startIndex) ?? (0, _univerjs_core.getParagraphContentStartOffset)(body, paragraph),
...
function getHeaderFooterOwnerSectionIds(documentDataModel, segmentId, sections = getDocumentPermissionResolverIndex(documentDataModel).topLevelSections) {
	...
	sections.forEach((section, sectionIndex) => {
		if (HEADER_FOOTER_REFERENCE_KEYS.some((key) => (0, _univerjs_core.resolveSectionHeaderFooterReference)(snapshot.documentStyle, sections, sectionIndex, key).segmentId === segmentId)) result.add(section.sectionId);
	});
	return [...result];
}
...
let DocPermissionController = class DocPermissionController extends _univerjs_core.Disposable {
	constructor(_injector, _commandService, _permissionService, _univerInstanceService) {
		...
		this._univerInstanceService = _univerInstanceService;
		this._univerInstanceService.getAllUnitsForType(_univerjs_core.UniverInstanceType.UNIVER_DOC).forEach((unit) => this._registerUnitPermissionPoints(unit.getUnitId()));
		this.disposeWithMe(this._univerInstanceService.getTypeOfUnitAdded$(_univerjs_core.UniverInstanceType.UNIVER_DOC).subscribe(({ unit }) => this._registerUnitPermissionPoints(unit.getUnitId())));
		this.disposeWithMe(this._commandService.beforeCommandExecuted((commandInfo, options) => {
			this._check(commandInfo, options);
		}));
		this.disposeWithMe(this._univerInstanceService.getTypeOfUnitDisposed$(_univerjs_core.UniverInstanceType.UNIVER_DOC).subscribe((unit) => {
			this._injector.get(_univerjs_core.ObjectPermissionService).clearUnit(unit.getUnitId());
	...
	_registerUnitPermissionPoints(unitId) {
		...
		this._injector.get(_univerjs_core.ObjectPermissionService).initializeUnit({
			unitId,
			objectId: unitId,
			objectType: _univerjs_protocol.UnitObject.Document
		});
	}
	_check(commandInfo, options) {
		...
		const unitId = getUnitId(commandInfo, options) ?? (unitAction ? (_this$_univerInstance = this._univerInstanceService.getCurrentUnitOfType(_univerjs_core.UniverInstanceType.UNIVER_DOC)) === null || _this$_univerInstance === void 0 ? void 0 : _this$_univerInstance.getUnitId() : void 0);
		if (!unitId || this._univerInstanceService.getUnitType(unitId) !== _univerjs_core.UniverInstanceType.UNIVER_DOC) return;
		if (unitAction) {
			if (!getDocumentPermissionValue(this._permissionService, unitId, unitId, unitAction)) {
				this._injector.get(_univerjs_core.ErrorService).emitPermissionDenied(unitId);
				throw new _univerjs_core.CustomCommandExecutionError(`Document ${_univerjs_protocol.UnitAction[unitAction]} permission denied.`);
			}
			return;
		}
		...
		if (!canEditDocumentTargets(this._permissionService, unitId, targetObjectIds)) {
			this._injector.get(_univerjs_core.ErrorService).emitPermissionDenied(unitId, targetObjectIds);
			throw new _univerjs_core.CustomCommandExecutionError("Document edit permission denied.");
		}
	}
...
function getDocumentUnitAction(commandId) {
	if (commandId === "doc.command.copy-current-paragraph" || commandId === "docs-table.command.copy-selection") return _univerjs_protocol.UnitAction.Copy;
	if (commandId === "docs.operation.print") return _univerjs_protocol.UnitAction.Print;
	if (commandId === "docs-exchange-client.operation.export-doc") return _univerjs_protocol.UnitAction.Export;
	if (commandId === "docs.operation.start-add-comment" || commandId === "docs.operation.add-drawing-comment" || commandId.startsWith("docs.command.") && commandId.includes("comment") || commandId.startsWith("thread-comment.command.") || commandId.startsWith("thread-comment.mutation.")) return _univerjs_protocol.UnitAction.Comment;
}
function resolveTextTargets(documentDataModel, commandInfo) {
	...
	if (id === DeleteTextCommand.id) {
		const params = commandInfo.params;
		const isDeleteLeft = params.direction === _univerjs_core.DeleteDirection.LEFT;
...
let UniverDocsPlugin = class UniverDocsPlugin extends _univerjs_core.Plugin {
	...
	onStarting() {
		...
		this.disposeWithMe(this._injector.get(_univerjs_core.ICommandService).registerCommand(SetDocumentPermissionRuleMutation));
		this.disposeWithMe(this._injector.get(_univerjs_core.ObjectPermissionService).registerRuleModel(_univerjs_protocol.UnitObject.Document, this._injector.get(DocumentPermissionRuleModel), SetDocumentPermissionRuleMutation.id));
...
function yieldForWorkerMessage() {
	return new Promise((resolve) => {
		(0, _univerjs_core.requestImmediateMacroTask)(() => resolve());
	});
}
function createDocsLayoutWorkerMessageProtocol$1(performanceTracker) {
	return {
		send(message) {
			const startedAt = _univerjs_core.Tools.now();
			postMessage(message);
			performanceTracker.recordResponse(message, _univerjs_core.Tools.now() - startedAt);
		},
...
var DocsLayoutWorkerRuntime = class {
	...
	async getCapabilities() {
		const metrics = _univerjs_engine_render.FontCache.getMeasureText(FONT_PROBE_CONTENT, FONT_PROBE_STYLE);
		return {
			protocolVersion: 4,
			executor: "worker",
	...
	async createSession(request) {
		...
		const dataModel = new _univerjs_core.DocumentDataModel(request.snapshot);
		const localeService = new _univerjs_core.LocaleService();
	...
	async startLayout(request) {
		...
		if (mount == null) {
			...
			mount = {
				...
				layoutSession: new _univerjs_engine_render.DocumentLayoutSession(session.dataModel, session.localeService),
				customBlockViewports,
	...
	_runWithCustomBlockViewports(session, mount, run) {
		const unregister = (0, _univerjs_engine_render.setDocsCustomBlockRenderViewportProvider)((unitId, blockId) => unitId === session.dataModel.getUnitId() ? mount.customBlockViewports[blockId] : null);
		try {
			return run();
		} finally {
			unregister();
...
function startDocsLayoutWorker() {
	...
	const channelService = new _univerjs_rpc.ChannelService(createDocsLayoutWorkerMessageProtocol$1(performanceTracker));
	channelService.registerChannel(DOCS_LAYOUT_WORKER_CHANNEL, (0, _univerjs_rpc.fromModule)(new DocsLayoutWorkerRuntime(performanceTracker)));
...
function createDocsLayoutWorkerMessageProtocol(worker, performanceTracker) {
	return {
		send(message) {
			...
			performanceTracker.recordRequest(message, _univerjs_core.Tools.now() - startedAt);
		},
		onMessage: new rxjs.Observable((subscriber) => {
			const handler = (event) => subscriber.next(event.data);
...
var DocsLayoutWorkerClientService = class extends _univerjs_core.Disposable {
	...
	createSession(request) {
		...
		const capturedRequest = _univerjs_core.Tools.deepClone(request);
	...
	async _replaceRuntime() {
		...
		const channelService = new _univerjs_rpc.ChannelService(createDocsLayoutWorkerMessageProtocol(worker, this._performanceTracker));
		this._worker = worker;
		this._channelService = channelService;
		this._runtime = (0, _univerjs_rpc.toModule)(channelService.requestChannel(DOCS_LAYOUT_WORKER_CHANNEL));
	...
	async _verifyCapabilities() {
		...
		const mainMetrics = _univerjs_engine_render.FontCache.getMeasureText(capabilities.fontProbe.content, capabilities.fontProbe.font);
		if ([
...
let UniverDocsLayoutWorkerPlugin = class UniverDocsLayoutWorkerPlugin extends _univerjs_core.Plugin {
	constructor(_config = defaultPluginDocsLayoutWorkerConfig, _injector, _layoutExecutorService, _configService) {
		...
		this._configService = _configService;
		const { ...rest } = (0, _univerjs_core.merge)({}, defaultPluginDocsLayoutWorkerConfig, this._config);
...
UniverDocsLayoutWorkerPlugin = __decorate([
	(0, _univerjs_core.DependentOn)(UniverDocsPlugin),
	__decorateParam(1, (0, _univerjs_core.Inject)(_univerjs_core.Injector)),
	__decorateParam(2, (0, _univerjs_core.Inject)(DocLayoutExecutorService)),
...
exports.DOCUMENT_UNIT_PERMISSION_ACTIONS = DOCUMENT_UNIT_PERMISSION_ACTIONS;
...
exports.DocHistoryAction = DocHistoryAction;
...
Object.defineProperty(exports, 'DocLayoutExecutorService', {
  enumerable: true,
  get: function () {
    return DocLayoutExecutorService;
  }
});
exports.DocLayoutExecutorState = DocLayoutExecutorState;
exports.DocLayoutExecutorType = DocLayoutExecutorType;
exports.DocLayoutSessionStatus = DocLayoutSessionStatus;
...
exports.DocsLayoutWorkerCapabilityError = DocsLayoutWorkerCapabilityError;
exports.DocsLayoutWorkerClientService = DocsLayoutWorkerClientService;
...
exports.SetDocumentPermissionCommand = SetDocumentPermissionCommand;
exports.SetDocumentPermissionRuleMutation = SetDocumentPermissionRuleMutation;
...
Object.defineProperty(exports, 'UniverDocsLayoutWorkerPlugin', {
  enumerable: true,
  get: function () {
    return UniverDocsLayoutWorkerPlugin;
  }
});
...
exports.canEditDocumentTargets = canEditDocumentTargets;
...
exports.createDocumentPermissionPoint = createDocumentPermissionPoint;
...
exports.getDocumentDrawingSegmentId = getDocumentDrawingSegmentId;
exports.getDocumentEditTargetObjectIds = getDocumentEditTargetObjectIds;
exports.getDocumentEntityParentPermissionObjectIds = getDocumentEntityParentPermissionObjectIds;
exports.getDocumentEntityPermissionObjectId = getDocumentEntityPermissionObjectId;
exports.getDocumentParagraphParentPermissionObjectIds = getDocumentParagraphParentPermissionObjectIds;
exports.getDocumentParagraphPermissionObjectId = getDocumentParagraphPermissionObjectId;
exports.getDocumentPermissionValue = getDocumentPermissionValue;
exports.getDocumentSectionPermissionObjectId = getDocumentSectionPermissionObjectId;
...
exports.setDocumentPermissionValue = setDocumentPermissionValue;
exports.shouldUseInlineTextSelectionForDocsCustomBlockDrawing = shouldUseInlineTextSelectionForDocsCustomBlockDrawing;
exports.startDocsLayoutWorker = startDocsLayoutWorker;
exports.transformDocumentTextRanges = transformDocumentTextRanges;
