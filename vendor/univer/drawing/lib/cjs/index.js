...
var UnitDrawingService = class {
	...
	refreshTransform(updateParams) {
		updateParams.forEach((updateParam) => {
			...
			if (Object.prototype.hasOwnProperty.call(updateParam, "transforms")) param.transforms = updateParam.transforms;
			if (Object.prototype.hasOwnProperty.call(updateParam, "isMultiTransform")) param.isMultiTransform = updateParam.isMultiTransform;
	...
	getBatchRemoveOpInOder(removeParams) {
		...
		removeParams.sort((a, b) => {
			return (orderIndexMap.get(a.drawingId) ?? Number.NEGATIVE_INFINITY) - (orderIndexMap.get(b.drawingId) ?? Number.NEGATIVE_INFINITY);
		});
