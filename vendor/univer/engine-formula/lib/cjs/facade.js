...
let FFormula = class FFormula extends _univerjs_core_facade.FBase {
	...
	*/
	calculationResultApplied(callback) {
		const subscription = this._injector.get(_univerjs_engine_formula.FormulaCalculationSessionService).resultApplied$.subscribe((result) => {
	...
	*/
	onCalculationResultApplied(timeout) {
		return this._injector.get(_univerjs_engine_formula.FormulaCalculationSessionService).waitForLatestApplied(timeout);
	}
	/**
