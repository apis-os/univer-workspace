	...
	return (requestConfig, next) => {
		...
		return new rxjs.Observable((observer) => {
			const hook = {
				...
				config: getParamsFromRequest(requestConfig),
				active: true
			};
			hookList.push(hook);
			...
			fetchCheck(requestConfig).then((isFetch) => {
				if (isFetch) {
					...
					const batch = { hooks: currentHookList };
			...
			return () => {
				var _hook$batch;
				hook.active = false;
				(0, _univerjs_core.remove)(hookList, hook);
				if ((_hook$batch = hook.batch) === null || _hook$batch === void 0 ? void 0 : _hook$batch.hooks.every((hookItem) => !hookItem.active)) {
					var _hook$batch$subscript;
					(_hook$batch$subscript = hook.batch.subscription) === null || _hook$batch$subscript === void 0 || _hook$batch$subscript.unsubscribe();
				}
			};
		});
	};
	...
	*/
	...
	return (request, next) => {
		return new rxjs.Observable((observer) => {
			...
			const teardown = () => {
				requestSubscription === null || requestSubscription === void 0 || requestSubscription.unsubscribe();
