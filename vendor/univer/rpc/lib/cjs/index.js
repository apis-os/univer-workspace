...
let WebWorkerRemoteInstanceService = class WebWorkerRemoteInstanceService {
	...
	whenReady() {
		if (this._lifecycleService.stage >= _univerjs_core.LifecycleStages.Ready) return Promise.resolve(true);
		return this._lifecycleService.onStage(_univerjs_core.LifecycleStages.Ready).then(() => true);
	}
	async syncMutation(params, options) {
...
let DataSyncPrimaryController = class DataSyncPrimaryController extends _univerjs_core.RxDisposable {
	...
	*/
	syncUnit(unitId) {
		...
		const unit = this._univerInstanceService.getUnit(unitId, _univerjs_core.UniverInstanceType.UNIVER_SHEET) ?? this._univerInstanceService.getUnit(unitId, _univerjs_core.UniverInstanceType.UNIVER_BASE);
	...
	_init() {
		...
		this.disposeWithMe(this._commandService.onCommandExecuted((commandInfo, options) => {
			...
			if (type === _univerjs_core.CommandType.MUTATION && (!unitId || this._syncingUnits.has(unitId)) && !(options === null || options === void 0 ? void 0 : options.fromSync) && this._syncingMutations.has(id)) this.syncMutation(commandInfo, options);
		}));
	}
};
