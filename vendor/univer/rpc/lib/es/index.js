import { CommandType, Disposable, ICommandService, IConfigService, ILogService, IUniverInstanceService, Inject, Injector, LifecycleService, LifecycleStages, Plugin, RxDisposable, UniverInstanceType, createIdentifier, merge, toDisposable } from "@univerjs/core";
...
let WebWorkerRemoteInstanceService = class WebWorkerRemoteInstanceService {
	constructor(_univerInstanceService, _commandService, _logService, _lifecycleService) {
...
var ChannelClient = class extends RxDisposable {
	...
	dispose() {
		...
		const error = /* @__PURE__ */ new Error("[ChannelClient]: client is disposed!");
		this._initialized.error(error);
		for (const responseHandler of this._pendingRequests.values()) responseHandler.dispose(error);
...
var ChannelServer = class extends RxDisposable {
	...
	dispose() {
		super.dispose();
		for (const subscription of this._subscriptions.values()) subscription.unsubscribe();
...
let DataSyncPrimaryController = class DataSyncPrimaryController extends RxDisposable {
	constructor(_injector, _commandService, _univerInstanceService, _rpcChannelService, _remoteSyncService) {
		...
		_defineProperty(this, "_remoteReady", Promise.resolve(true));
	...
	*/
	syncUnit(unitId) {
		...
		const unit = this._univerInstanceService.getUnit(unitId, UniverInstanceType.UNIVER_SHEET) ?? this._univerInstanceService.getUnit(unitId, UniverInstanceType.UNIVER_BASE);
	...
	async syncMutation(commandInfo, options) {
		const sync = () => this._remoteReady.then(() => this._remoteInstanceService.syncMutation({ mutationInfo: commandInfo }, options));
	...
	_init() {
		...
		this.disposeWithMe(this._commandService.onCommandExecuted((commandInfo, options) => {
			...
			if (type === CommandType.MUTATION && (!unitId || this._syncingUnits.has(unitId)) && !(options === null || options === void 0 ? void 0 : options.fromSync) && this._syncingMutations.has(id)) this.syncMutation(commandInfo, options);
		}));
	}
};
