...
const transformDate = (value, dateSystem, patternType) => {
	const timeAnchorYear = dateSystem === _univerjs_core.DateSystem.Date1904 ? 1904 : 1900;
	if (value === void 0 || value === null) return patternType === "time" ? (0, _univerjs_core.dateKit)(new Date(timeAnchorYear, 0, 1)) : void 0;
	if (typeof value === "boolean") return;
	if (value === "") return patternType === "time" ? (0, _univerjs_core.dateKit)(new Date(timeAnchorYear, 0, 1)) : (0, _univerjs_core.dateKit)();
	if (typeof value === "number" || !Number.isNaN(+value)) {
		const parts = (0, _univerjs_core.excelSerialToDateTimeParts)(Number(value), { dateSystem });
		...
		return (0, _univerjs_core.dateKit)(new Date(year, month - 1, day, parts.hours, parts.minutes, parts.seconds, Math.round(parts.fractionalSecond * 1e3)));
	}
	const date = (0, _univerjs_core.dateKit)(value);
	if (date.isValid()) return date;
};
function getDefaultFormat(patternType, format) {
	if (patternType === "duration" && _univerjs_core.numfmt.getFormatDateInfo(format).isDuration) return format;
...
let DataValidationDropdownManagerService = class DataValidationDropdownManagerService extends _univerjs_core.Disposable {
	...
	showDropdown(param) {
		...
		const originalValue = (0, _univerjs_sheets_data_validation.getCellValueOrigin)(worksheet.getCellRaw(row, col));
		...
		const originalParts = originalSerial == null ? null : (0, _univerjs_core.excelSerialToDateTimeParts)(originalSerial, { dateSystem });
		...
		const serialProps = unsupportedValue == null ? {} : {
			...
			onSerialChange: (newValue) => {
				if (newValue == null || !(0, _univerjs_core.excelSerialToDateTimeParts)(newValue, { dateSystem })) return Promise.resolve(false);
				return handleSerialSave(newValue, validator.dropdownType === _univerjs_data_validation.DataValidatorDropdownType.DATE ? "date" : validator.dropdownType === _univerjs_data_validation.DataValidatorDropdownType.TIME ? "time" : "datetime", String(newValue));
			}
		};
		switch (validator.dropdownType) {
			...
			case _univerjs_data_validation.DataValidatorDropdownType.TIME: {
				if (_univerjs_core.numfmt.getFormatDateInfo(format).isDuration) {
					dropdownParam = {
						location,
						type: "datepicker",
						props: {
...
function DataValidationList(props) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		}, rule.uid)), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(_univerjs_design.ActionRow, {
