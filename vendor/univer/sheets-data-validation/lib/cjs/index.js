...
let SheetDataValidationModel = class SheetDataValidationModel extends _univerjs_core.Disposable {
	...
	validator(rule, pos, _onCompete) {
		...
		if (validator) {
			...
			onCompete(current ?? _univerjs_core.DataValidationStatus.VALID, false);
			return current ?? _univerjs_core.DataValidationStatus.VALID;
		} else {
...
var CheckboxValidator = class extends BaseSheetValidator {
	...
	async isValidType(cellInfo, formula, rule) {
		...
		return _univerjs_core.Tools.isDefine(value) && (String(value) === String(formula1) || String(value) === String(formula2) || String(value) === String(originFormula1 ?? "") || String(value) === String(originFormula2 ?? ""));
	}
...
const transformDate2SerialNumber = (value, options = {}) => {
	...
	return (_numfmt$parseDate = _univerjs_core.numfmt.parseDate(value, options)) === null || _numfmt$parseDate === void 0 ? void 0 : _numfmt$parseDate.v;
};
function pad(value) {
...
var DateValidator = class extends BaseSheetValidator {
	...
	_getParseOptions(unitId, workbook) {
		const targetWorkbook = workbook ?? (unitId ? this._univerInstanceService.getUnit(unitId, _univerjs_core.UniverInstanceType.UNIVER_SHEET) : void 0);
		return targetWorkbook ? {
			locale: (0, _univerjs_core.getNumfmtLocaleTag)(targetWorkbook.getSnapshot().locale),
			dateSystem: targetWorkbook.getDateSystem()
		} : { dateSystem: _univerjs_core.DateSystem.Date1900 };
	}
	async parseFormula(rule, unitId, subUnitId, row, column) {
		...
		return {
			formula1: transformDate2SerialNumber((0, _univerjs_core.isFormulaString)(formula1) ? formulaResult1 === null || formulaResult1 === void 0 ? void 0 : formulaResult1.v : formula1, this._getParseOptions(unitId)),
			formula2: transformDate2SerialNumber((0, _univerjs_core.isFormulaString)(formula2) ? formulaResult2 === null || formulaResult2 === void 0 ? void 0 : formulaResult2.v : formula2, this._getParseOptions(unitId)),
			isFormulaValid
		};
	}
	async isValidType(info) {
		...
		if (typeof value === "number" && typeof interceptValue === "string") return Boolean(_univerjs_core.numfmt.parseDate(interceptValue, options));
		if (typeof interceptValue === "string") return Boolean(_univerjs_core.numfmt.parseDate(interceptValue, options));
		return false;
	}
	_validatorSingleFormula(formula, options) {
		return !_univerjs_core.Tools.isBlank(formula) && ((0, _univerjs_core.isFormulaString)(formula) || !Number.isNaN(+formula) || Boolean(formula && _univerjs_core.numfmt.parseDate(formula, options)));
	}
	validatorFormula(rule, unitId, subUnitId) {
	...
	normalizeFormula(rule, unitId, _subUnitId) {
		...
		const normlizeSingleFormula = (formula) => {
			...
			const serial = !Number.isNaN(+formula) ? +formula : (_numfmt$parseDate2 = _univerjs_core.numfmt.parseDate(formula, options)) === null || _numfmt$parseDate2 === void 0 ? void 0 : _numfmt$parseDate2.v;
			if (typeof serial !== "number") return "";
			const date = (0, _univerjs_core.excelSerialToDateTimeParts)(serial, { dateSystem: options.dateSystem ?? _univerjs_core.DateSystem.Date1900 });
			if (!date) return "";
...
var ListValidator = class extends BaseSheetValidator {
	...
	validatorFormula(rule, unitId, subUnitId) {
		...
		const isIntersects = isRuleIntersects(rule, ((_this$_univerInstance = this._univerInstanceService.getUnit(unitId, _univerjs_core.UniverInstanceType.UNIVER_SHEET)) === null || _this$_univerInstance === void 0 || (_this$_univerInstance = _this$_univerInstance.getSheetBySheetId(subUnitId)) === null || _this$_univerInstance === void 0 ? void 0 : _this$_univerInstance.getName()) ?? "");
		return {
	...
	getExtraStyle(rule, value, { style: defaultStyle }) {
		const tb = (defaultStyle.tb !== _univerjs_core.WrapStrategy.OVERFLOW ? defaultStyle.tb : _univerjs_core.WrapStrategy.CLIP) ?? _univerjs_core.WrapStrategy.WRAP;
	...
	_getUnitAndSubUnit(currentUnitId, currentSubUnitId) {
		const workbook = (currentUnitId ? this._univerInstanceService.getUnit(currentUnitId, _univerjs_core.UniverInstanceType.UNIVER_SHEET) : void 0) ?? this._univerInstanceService.getCurrentUnitOfType(_univerjs_core.UniverInstanceType.UNIVER_SHEET);
	...
	async validatorRanges(unitId, subUnitId, ranges) {
		...
		*/
		if (mergeCells.length) mergeCells.forEach(({ resultRowIndex, resultColIndex, row, col }) => {
			if (result[resultRowIndex][resultColIndex] === _univerjs_core.DataValidationStatus.VALIDATING) result[resultRowIndex][resultColIndex] = this._dataValidationCacheService.getValue(unitId, subUnitId, row, col) ?? _univerjs_core.DataValidationStatus.VALID;
		});
		return result;
	}
...
UniverSheetsDataValidationPlugin = __decorate([
	(0, _univerjs_core.DependentOn)(_univerjs_data_validation.UniverDataValidationPlugin, _univerjs_engine_formula.UniverFormulaEnginePlugin, _univerjs_sheets.UniverSheetsPlugin, _univerjs_sheets_formula.UniverSheetsFormulaPlugin),
	__decorateParam(1, (0, _univerjs_core.Inject)(_univerjs_core.Injector)),
