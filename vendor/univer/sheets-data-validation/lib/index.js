import { CellValueType, CommandType, DataValidationOperator, DataValidationRenderMode, DataValidationStatus, DataValidationType, DateSystem, DependentOn, Disposable, ICommandService, IConfigService, IPermissionService, IUndoRedoService, IUniverInstanceService, Inject, Injector, LifecycleService, LifecycleStages, ObjectMatrix, Plugin, RBush, Range, Rectangle, RxDisposable, Tools, UniverInstanceType, WrapStrategy, bufferDebounceTime, debounce, excelSerialToDateTimeParts, generateRandomId, getIntersectRange, getNumfmtLocaleTag, getOriginCellValue, isFormulaString, isRangesEqual, merge, numfmt, sequenceExecute, toDisposable } from "@univerjs/core";
...
import { ERROR_TYPE_SET, LexerTreeBuilder, OtherFormulaBizType, RegisterOtherFormulaService, UniverFormulaEnginePlugin, deserializeRangeWithSheet, deserializeRangeWithSheetWithCache, isReferenceString, operatorToken, sequenceNodeType } from "@univerjs/engine-formula";
import { ClearSelectionAllCommand, CopySheetCommand, RefRangeService, RemoveSheetCommand, RemoveSheetMutation, SetRangeValuesMutation, SetRangeValuesUndoMutationFactory, SheetInterceptorService, SheetsSelectionsService, UniverSheetsPlugin, WorksheetViewPermission, deserializeListOptions, getSheetCommandTarget, handleCommonDefaultRangeChangeWithEffectRefCommands } from "@univerjs/sheets";
...
function shouldOffsetFormulaByRange(type, validatorRegistryService) {
	...
	return (validator === null || validator === void 0 ? void 0 : validator.offsetFormulaByRange) ?? false;
}
...
var BaseSheetValidator = class extends BaseDataValidator {
	...
	generateRuleName(rule) {
		...
		const ruleName = this.localeService.t(OperatorTitleMap[rule.operator]).replace(FORMULA1$1, rule.formula1 ?? "").replace(FORMULA2$1, rule.formula2 ?? "");
	...
	generateRuleErrorMessage(rule, _position) {
		...
		return `${this.localeService.t(OperatorErrorTitleMap[rule.operator]).replace(FORMULA1$1, rule.formula1 ?? "").replace(FORMULA2$1, rule.formula2 ?? "")}`;
	}
};
...
var CheckboxValidator = class extends BaseSheetValidator {
	...
	async isValidType(cellInfo, formula, rule) {
		...
		return Tools.isDefine(value) && (String(value) === String(formula1) || String(value) === String(formula2) || String(value) === String(originFormula1 ?? "") || String(value) === String(originFormula2 ?? ""));
	}
...
const transformDate2SerialNumber = (value, options = {}) => {
	var _numfmt$parseDate;
	if (value === void 0 || value === null || typeof value === "boolean") return;
	if (typeof value === "number" || !Number.isNaN(+value)) return +value;
	return (_numfmt$parseDate = numfmt.parseDate(value, options)) === null || _numfmt$parseDate === void 0 ? void 0 : _numfmt$parseDate.v;
};
function pad(value) {
...
var DateValidator = class extends BaseSheetValidator {
	...
	_getParseOptions(unitId, workbook) {
		const targetWorkbook = workbook ?? (unitId ? this._univerInstanceService.getUnit(unitId, UniverInstanceType.UNIVER_SHEET) : void 0);
		return targetWorkbook ? {
			locale: getNumfmtLocaleTag(targetWorkbook.getSnapshot().locale),
			dateSystem: targetWorkbook.getDateSystem()
	...
	async parseFormula(rule, unitId, subUnitId, row, column) {
		...
		return {
			formula1: transformDate2SerialNumber(isFormulaString(formula1) ? formulaResult1 === null || formulaResult1 === void 0 ? void 0 : formulaResult1.v : formula1, this._getParseOptions(unitId)),
			formula2: transformDate2SerialNumber(isFormulaString(formula2) ? formulaResult2 === null || formulaResult2 === void 0 ? void 0 : formulaResult2.v : formula2, this._getParseOptions(unitId)),
			isFormulaValid
		};
	}
	async isValidType(info) {
		...
		if (typeof value === "number" && typeof interceptValue === "string") return Boolean(numfmt.parseDate(interceptValue, options));
		if (typeof interceptValue === "string") return Boolean(numfmt.parseDate(interceptValue, options));
		return false;
	}
	_validatorSingleFormula(formula, options) {
	...
	normalizeFormula(rule, unitId, _subUnitId) {
		const { formula1, formula2, bizInfo } = rule;
		const options = this._getParseOptions(unitId);
		const normlizeSingleFormula = (formula) => {
			var _numfmt$parseDate2;
			if (!formula) return formula;
			const serial = !Number.isNaN(+formula) ? +formula : (_numfmt$parseDate2 = numfmt.parseDate(formula, options)) === null || _numfmt$parseDate2 === void 0 ? void 0 : _numfmt$parseDate2.v;
			...
			return (bizInfo === null || bizInfo === void 0 ? void 0 : bizInfo.showTime) ? `${dateText} ${pad(date.hours)}:${pad(date.minutes)}:${pad(date.seconds)}` : dateText;
		};
		return {
	...
	generateRuleErrorMessage(rule, pos) {
		...
		return `${this.localeService.t(DateOperatorErrorTitleMap[rule.operator]).replace(FORMULA1, transformedFormula1 ?? "").replace(FORMULA2, transformedFormula2 ?? "")}`;
	}
};
...
var DecimalValidator = class extends BaseSheetValidator {
	...
	generateRuleErrorMessage(rule, position) {
		...
		return `${this.localeService.t(OperatorErrorTitleMap[rule.operator]).replace(FORMULA1, transformedFormula1 ?? "").replace(FORMULA2, transformedFormula2 ?? "")}`;
	}
};
...
var ListValidator = class extends BaseSheetValidator {
	...
	validatorFormula(rule, unitId, subUnitId) {
		...
		const isIntersects = isRuleIntersects(rule, ((_this$_univerInstance = this._univerInstanceService.getUnit(unitId, UniverInstanceType.UNIVER_SHEET)) === null || _this$_univerInstance === void 0 || (_this$_univerInstance = _this$_univerInstance.getSheetBySheetId(subUnitId)) === null || _this$_univerInstance === void 0 ? void 0 : _this$_univerInstance.getName()) ?? "");
		return {
	...
	getExtraStyle(rule, value, { style: defaultStyle }) {
		...
		if (rule.type === DataValidationType.LIST && (rule.renderMode === DataValidationRenderMode.ARROW || rule.renderMode === DataValidationRenderMode.TEXT)) {
			const color = this.getListWithColorMap(rule)[`${value ?? ""}`];
			if (color) return {
				bg: { rgb: color },
				tb
			};
	...
	_getUnitAndSubUnit(currentUnitId, currentSubUnitId) {
		const workbook = (currentUnitId ? this._univerInstanceService.getUnit(currentUnitId, UniverInstanceType.UNIVER_SHEET) : void 0) ?? this._univerInstanceService.getCurrentUnitOfType(UniverInstanceType.UNIVER_SHEET);
		if (!workbook) return null;
		const worksheet = (currentSubUnitId ? workbook.getSheetBySheetId(currentSubUnitId) : void 0) ?? workbook.getActiveSheet();
...
var TextLengthValidator = class extends BaseSheetValidator {
	...
	generateRuleErrorMessage(rule, pos) {
		if (!rule.operator) return this.titleStr;
		const { transformedFormula1, transformedFormula2 } = getTransformedFormula(this._lexerTreeBuilder, rule, pos);
		return `${this.localeService.t(TextLengthErrorTitleMap[rule.operator]).replace(FORMULA1, transformedFormula1 ?? "").replace(FORMULA2, transformedFormula2 ?? "")}`;
	}
};
...
function isBlankCell(cellData) {
	var _cellData$p$body;
	if (!cellData) return true;
	if (!cellData.p) return Tools.isBlank(cellData.v);
	return !(((_cellData$p$body = cellData.p.body) === null || _cellData$p$body === void 0 ? void 0 : _cellData$p$body.dataStream) ?? "").slice(0, -2).trim();
}
...
const ClearRangeDataValidationCommand = {
	...
	handler(accessor, params) {
		...
		const undoRedoService = accessor.get(IUndoRedoService);
		const matrix = sheetDataValidationModel.getRuleObjectMatrix(unitId, subUnitId).clone();
		const ruleIds = types ? new Set(sheetDataValidationModel.getRules(unitId, subUnitId).filter((rule) => types.includes(rule.type)).map((rule) => rule.uid)) : void 0;
...
let DataValidationRefRangeController = class DataValidationRefRangeController extends Disposable {
	...
	registerFormula(unitId, subUnitId, rule) {
		...
		const disposeSet = this._disposableMap.get(id) ?? /* @__PURE__ */ new Set();
	...
	register(unitId, subUnitId, rule) {
		...
		const id = this._getIdWithUnitId(unitId, subUnitId, rule.uid);
		const current = this._disposableMap.get(id) ?? /* @__PURE__ */ new Set();
	...
	async validatorRanges(unitId, subUnitId, ranges) {
		...
		*/
		if (mergeCells.length) mergeCells.forEach(({ resultRowIndex, resultColIndex, row, col }) => {
			if (result[resultRowIndex][resultColIndex] === DataValidationStatus.VALIDATING) result[resultRowIndex][resultColIndex] = this._dataValidationCacheService.getValue(unitId, subUnitId, row, col) ?? DataValidationStatus.VALID;
		});
		return result;
	}
...
UniverSheetsDataValidationPlugin = __decorate([
	DependentOn(UniverDataValidationPlugin, UniverFormulaEnginePlugin, UniverSheetsPlugin, UniverSheetsFormulaPlugin),
	__decorateParam(1, Inject(Injector)),
