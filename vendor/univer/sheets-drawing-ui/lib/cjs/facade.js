...
var FWorksheetDrawingUIMixin = class extends _univerjs_sheets_facade.FWorksheet {
	...
	updateFloatDom(id, config) {
		...
		const newParam = {
			...
			sheetTransform: config.position ? (0, _univerjs_sheets_drawing.transformToDrawingPosition)(config.position, skeleton) ?? drawingParm.sheetTransform : drawingParm.sheetTransform,
			transform: {
				...drawingParm.transform,
				...config.position
			},
			axisAlignSheetTransform: config.position ? (0, _univerjs_sheets_drawing.transformToAxisAlignPosition)(config.position, skeleton) ?? drawingParm.sheetTransform : drawingParm.sheetTransform
		};
	...
	batchUpdateFloatDoms(updates) {
		...
		for (const update of updates) {
			...
			const newParam = {
				...
				sheetTransform: update.config.position ? (0, _univerjs_sheets_drawing.transformToDrawingPosition)(update.config.position, skeleton) ?? drawingParm.sheetTransform : drawingParm.sheetTransform,
				transform: {
					...drawingParm.transform,
					...update.config.position
				},
				axisAlignSheetTransform: update.config.position ? (0, _univerjs_sheets_drawing.transformToAxisAlignPosition)(update.config.position, skeleton) ?? drawingParm.sheetTransform : drawingParm.sheetTransform
			};
	...
	async saveCellImagesAsync(options, ranges) {
		...
		const fileNameParts = [];
		const useCellAddress = (options === null || options === void 0 ? void 0 : options.useCellAddress) ?? true;
	...
	_initialize(injector) {
		...
		this.disposeWithMe(this.registerEventHandler(this.Event.FloatDomDeleted, () => commandService.onCommandExecuted((commandInfo) => {
			...
			for (let i = 0; i < drawings.length; i++) {
				...
				if (drawing.drawingType === _univerjs_core.DrawingTypeEnum.DRAWING_DOM) floatDomDrawingIds.push(drawing.drawingId);
			}
