...
var FWorksheetDrawingUIMixin = class extends FWorksheet {
	...
	updateFloatDom(id, config) {
		...
		const newParam = {
			...
			sheetTransform: config.position ? transformToDrawingPosition(config.position, skeleton) ?? drawingParm.sheetTransform : drawingParm.sheetTransform,
			transform: {
				...drawingParm.transform,
				...config.position
			},
			axisAlignSheetTransform: config.position ? transformToAxisAlignPosition(config.position, skeleton) ?? drawingParm.sheetTransform : drawingParm.sheetTransform
		};
	...
	batchUpdateFloatDoms(updates) {
		...
		for (const update of updates) {
			...
			const newParam = {
				...
				sheetTransform: update.config.position ? transformToDrawingPosition(update.config.position, skeleton) ?? drawingParm.sheetTransform : drawingParm.sheetTransform,
				transform: {
					...drawingParm.transform,
					...update.config.position
				},
				axisAlignSheetTransform: update.config.position ? transformToAxisAlignPosition(update.config.position, skeleton) ?? drawingParm.sheetTransform : drawingParm.sheetTransform
			};
