...
function convertSheetImageToFOverGridImage(sheetImage, skeleton) {
	...
	const { width, height } = (0, _univerjs_sheets.convertPositionSheetOverGridToAbsolute)(sheetImage.unitId, sheetImage.subUnitId, {
		from,
		to
	}, skeleton);
	return {
...
function convertFOverGridImageToSheetImage(fOverGridImage, sheetSkeletonService) {
	...
	const { sheetTransform, transform } = (0, _univerjs_sheets.convertPositionCellToSheetOverGrid)(fOverGridImage.unitId, fOverGridImage.subUnitId, {
		column: fromColumn,
		columnOffset: fromColumnOffset,
		row: fromRow,
		rowOffset: fromRowOffset
...
let FOverGridImage = class FOverGridImage extends _univerjs_core_facade.FBase {
	...
	*/
	remove() {
		return this._commandService.syncExecuteCommand(_univerjs_sheets_drawing.RemoveSheetDrawingCommand.id, {
			unitId: this._image.unitId,
	...
	*/
	setForward() {
		return this._commandService.syncExecuteCommand(_univerjs_sheets_drawing.SetDrawingArrangeCommand.id, {
			unitId: this._image.unitId,
			subUnitId: this._image.subUnitId,
			drawingIds: [this._image.drawingId],
			arrangeType: _univerjs_core.ArrangeTypeEnum.forward
		});
	}
	/**
	...
	*/
	setBackward() {
		return this._commandService.syncExecuteCommand(_univerjs_sheets_drawing.SetDrawingArrangeCommand.id, {
			...
			subUnitId: this._image.subUnitId,
			drawingIds: [this._image.drawingId],
			arrangeType: _univerjs_core.ArrangeTypeEnum.backward
		});
	}
	/**
	...
	*/
	setBack() {
		return this._commandService.syncExecuteCommand(_univerjs_sheets_drawing.SetDrawingArrangeCommand.id, {
			...
			subUnitId: this._image.subUnitId,
			drawingIds: [this._image.drawingId],
			arrangeType: _univerjs_core.ArrangeTypeEnum.back
		});
	}
	/**
	...
	*/
	setFront() {
		return this._commandService.syncExecuteCommand(_univerjs_sheets_drawing.SetDrawingArrangeCommand.id, {
			...
			subUnitId: this._image.subUnitId,
			drawingIds: [this._image.drawingId],
			arrangeType: _univerjs_core.ArrangeTypeEnum.front
		});
	}
};
...
var FWorksheetDrawingMixin = class extends _univerjs_sheets_facade.FWorksheet {
	get _sheetDrawingService() {
		return this._injector.get(_univerjs_sheets_drawing.ISheetDrawingService);
	}
	get _sheetSkeletonService() {
		return this._injector.get(_univerjs_sheets.SheetSkeletonService);
	}
	get _undoRedoService() {
		return this._injector.get(_univerjs_core.IUndoRedoService);
	}
	async insertImage(url, column, row, offsetX, offsetY) {
		...
		if (typeof url === "string") imageBuilder.setSource(url);
			...
			imageBuilder.setSource(base64, _univerjs_core.ImageSourceType.BASE64);
		}
		...
		return this._commandService.syncExecuteCommand(_univerjs_sheets_drawing.InsertSheetDrawingCommand.id, {
	...
	insertImages(sheetImages) {
		...
		this._commandService.syncExecuteCommand(_univerjs_sheets_drawing.InsertSheetDrawingCommand.id, {
			unitId: this._fWorkbook.getId(),
			drawings: param
		});
		return this;
	...
	deleteImages(sheetImages) {
		...
		this._commandService.syncExecuteCommand(_univerjs_sheets_drawing.RemoveSheetDrawingCommand.id, {
			unitId: this._fWorkbook.getId(),
			drawings
		});
		return this;
	...
	getImages() {
		...
		for (const drawingId in drawingData) {
			...
			if (drawing.drawingType !== _univerjs_core.DrawingTypeEnum.DRAWING_IMAGE) continue;
	...
	getImageById(id) {
		...
		if (drawing && drawing.drawingType === _univerjs_core.DrawingTypeEnum.DRAWING_IMAGE) return this._injector.createInstance(FOverGridImage, drawing);
		return null;
	}
	getActiveImages() {
	...
	groupDrawings(drawingIds, groupId = (0, _univerjs_core.generateRandomId)(10)) {
		...
		return this._applyGroupDrawingOperation([{
			parent: groupParam,
			children
		}], _univerjs_sheets_drawing.DrawingApplyType.GROUP) ? groupId : null;
	}
	ungroupDrawings(groupIds) {
		...
		for (const groupId of groupIds) {
			...
			if (!groupDrawing || groupDrawing.drawingType !== _univerjs_core.DrawingTypeEnum.DRAWING_GROUP) continue;
			...
			const children = sheetDrawingService.getDrawingsByGroup({
				...
				const newTransform = (0, _univerjs_engine_render.transformObjectOutOfGroup)(drawing.transform || {}, groupTransform, groupTransform.width || 0, groupTransform.height || 0, groupDrawing.groupBaseBound);
				return {
					...drawing,
					transform: {
						...drawing.transform,
		...
		return this._applyGroupDrawingOperation(groupParams, _univerjs_sheets_drawing.DrawingApplyType.UNGROUP);
	}
	...
	getDrawingParentGroup(drawingId) {
		...
		if (!groupDrawing || groupDrawing.drawingType !== _univerjs_core.DrawingTypeEnum.DRAWING_GROUP) return null;
		return groupDrawing;
	}
	...
	_applyGroupDrawingOperation(groupParams, type) {
		...
		if (commandService.syncExecuteCommand(_univerjs_sheets_drawing.SetDrawingApplyMutation.id, {
			...
			const inverseType = type === _univerjs_sheets_drawing.DrawingApplyType.GROUP ? _univerjs_sheets_drawing.DrawingApplyType.UNGROUP : _univerjs_sheets_drawing.DrawingApplyType.GROUP;
	...
	_invertGroupOperationObjects(groupParams, type) {
		return type === _univerjs_sheets_drawing.DrawingApplyType.GROUP ? groupParams.map((groupParam) => {
			...
			return {
				...
				children: children.map((child) => ({
					...child,
					transform: (0, _univerjs_engine_render.transformObjectOutOfGroup)(child.transform || {}, groupTransform, groupTransform.width || 0, groupTransform.height || 0, parent.groupBaseBound),
					groupId: void 0
				}))
			};
		}) : groupParams.map((groupParam) => {
			const { parent, children } = groupParam;
			const groupTransform = (0, _univerjs_engine_render.getGroupState)(0, 0, children.map((child) => child.transform || {}));
			return {
				parent: {
					...parent,
					transform: groupTransform
...
*/
var FSheetsDrawingEnumMixin = class extends _univerjs_core_facade.FEnum {
	get DrawingType() {
...
_univerjs_core_facade.FEnum.extend(FSheetsDrawingEnumMixin);

//#endregion
//#region src/facade/f-event.ts
var FSheetsDrawingEventNameMixin = class extends _univerjs_core_facade.FEventName {
...
_univerjs_core_facade.FEventName.extend(FSheetsDrawingEventNameMixin);
...
* @ignore
*/
var FUniverSheetsDrawingMixin = class extends _univerjs_core_facade.FUniver {
	/**
	* @ignore
	*/
	_initialize(injector) {
		...
		this.disposeWithMe(this.registerEventHandler(this.Event.BeforeOverGridImageSelect, () => commandService.beforeCommandExecuted((commandInfo) => {
			if (commandInfo.id !== _univerjs_drawing.SetDrawingSelectedOperation.id) return;
...
_univerjs_core_facade.FUniver.extend(FUniverSheetsDrawingMixin);

//#endregion
Object.defineProperty(exports, 'FOverGridImage', {
  enumerable: true,
  get: function () {
    return FOverGridImage;
  }
});