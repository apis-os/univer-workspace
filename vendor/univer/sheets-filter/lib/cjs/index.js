...
const notEquals = {
	...
	fn: (value, compare) => {
		if (typeof compare === "string") {
			...
			if (ensuredString && isWildCardString(compare)) return !_univerjs_core.regexp.createFromWildcard(compare).test(ensuredString);
...
const textMatch = { fn: (value, compare) => {
	...
	return _univerjs_core.regexp.createFromWildcard(compare).test(ensured);
} };
...
let SheetsFilterController = class SheetsFilterController extends _univerjs_core.Disposable {
	...
	_handleRemoveRowCommand(config, unitId, subUnitId) {
		...
		if (count === endRow - startRow + 1 || filterHeaderIsRemoved) {
			...
			const worksheet = (_this$_univerInstance = this._univerInstanceService.getUnit(unitId, _univerjs_core.UniverInstanceType.UNIVER_SHEET)) === null || _this$_univerInstance === void 0 ? void 0 : _this$_univerInstance.getSheetBySheetId(subUnitId);
