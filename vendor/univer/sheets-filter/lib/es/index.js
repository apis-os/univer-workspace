import { CellValueType, ColorKit, CommandType, DependentOn, Disposable, DisposableCollection, ErrorService, ICommandService, IConfigService, IResourceManagerService, IUndoRedoService, IUniverInstanceService, Inject, Injector, LocaleService, Optional, Plugin, Rectangle, Tools, UniverInstanceType, extractPureTextFromCell, fromCallback, isNumeric, merge, mergeSets, moveMatrixArray, regexp, sequenceExecute, touchDependencies } from "@univerjs/core";
import { CopySheetCommand, EffectRefRangId, INTERCEPTOR_POINT, InsertColCommand, InsertColMutation, InsertRowCommand, InsertRowMutation, MarkDirtyFilterChangeMutation, MoveColsMutation, MoveRangeCommand, MoveRowsCommand, RefRangeService, RemoveColCommand, RemoveColMutation, RemoveRowCommand, RemoveRowMutation, RemoveSheetCommand, SetRangeValuesMutation, SetWorksheetActiveOperation, SheetInterceptorService, SheetsSelectionsService, UniverSheetsPlugin, ZebraCrossingCacheController, expandToContinuousRange, getSheetCommandTarget, isSingleCellSelection } from "@univerjs/sheets";
...
import { IActiveDirtyManagerService, ISheetRowFilteredService, UniverFormulaEnginePlugin } from "@univerjs/engine-formula";
...
let SheetsFilterService = class SheetsFilterService extends Disposable {
	...
	*/
	...
	getFilterModel(unitId, subUnitId) {
		var _this$_filterModels$g;
		return ((_this$_filterModels$g = this._filterModels.get(unitId)) === null || _this$_filterModels$g === void 0 ? void 0 : _this$_filterModels$g.get(subUnitId)) ?? null;
	}
	...
	_initActiveFilterModel() {
		this.disposeWithMe(merge$1(fromCallback(this._commandService.onCommandExecuted.bind(this._commandService)).pipe(filter(([command]) => command.type === CommandType.MUTATION && FILTER_MUTATIONS.has(command.id))), this._univerInstanceService.getCurrentTypeOfUnit$(UniverInstanceType.UNIVER_SHEET).pipe(switchMap((workbook) => (workbook === null || workbook === void 0 ? void 0 : workbook.activeSheet$) ?? of(null)))).subscribe(() => this._updateActiveFilterModel()));
	}
...
let SheetsFilterController = class SheetsFilterController extends Disposable {
	...
	handleInsertColCommand(range, unitId, subUnitId) {
		...
		const filterRange = (filterModel === null || filterModel === void 0 ? void 0 : filterModel.getRange()) ?? null;
	...
	_handleRemoveRowCommand(config, unitId, subUnitId) {
		...
		if (count === endRow - startRow + 1 || filterHeaderIsRemoved) {
			...
			const worksheet = (_this$_univerInstance = this._univerInstanceService.getUnit(unitId, UniverInstanceType.UNIVER_SHEET)) === null || _this$_univerInstance === void 0 ? void 0 : _this$_univerInstance.getSheetBySheetId(subUnitId);
	...
	_initRowFilteredInterceptor() {
		this.disposeWithMe(this._sheetInterceptorService.intercept(INTERCEPTOR_POINT.ROW_FILTERED, { handler: (filtered, rowLocation) => {
			...
			return ((_this$_sheetsFilterSe2 = this._sheetsFilterService.getFilterModel(rowLocation.unitId, rowLocation.subUnitId)) === null || _this$_sheetsFilterSe2 === void 0 ? void 0 : _this$_sheetsFilterSe2.isRowFiltered(rowLocation.row)) ?? false;
		} }));
	}
...
let SheetsFilterSyncController = class SheetsFilterSyncController extends Disposable {
	...
	constructor(_sheetsFilterController, _commandService, _configService) {
		...
		if (config === null || config === void 0 ? void 0 : config.enableSyncSwitch) {
			...
			if (typeof config.enableSyncSwitch === "object") this.setEnabled(config.enableSyncSwitch.defaultValue ?? true);
		}
	}
	setEnabled(enabled) {
...
let SheetsFilterFormulaService = class SheetsFilterFormulaService extends Disposable {
	...
	_registerSheetRowFiltered() {
		this._sheetRowFilteredService.register((unitId, subUnitId, row) => {
			var _this$_sheetsFilterSe2;
			return ((_this$_sheetsFilterSe2 = this._sheetsFilterService.getFilterModel(unitId, subUnitId)) === null || _this$_sheetsFilterSe2 === void 0 ? void 0 : _this$_sheetsFilterSe2.isRowFiltered(row)) ?? false;
		});
	}
};
...
UniverSheetsFilterPlugin = __decorate([
