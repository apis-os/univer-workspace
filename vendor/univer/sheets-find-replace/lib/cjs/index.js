	...
	*/
	...
	_getReplacedCellData(match, worksheet, shouldReplaceFormula, findString, replaceString, replaceFlag) {
		...
		if (match.isFormula) {
			...
			return {
				f: currentContent.f.replace(_univerjs_core.regexp.createLiteralRegExp(findString, replaceFlag), replaceString),
				v: null
			};
		}
		...
		return { v: currentContent.v.toString().replace(_univerjs_core.regexp.createLiteralRegExp(findString, replaceFlag), replaceString) };
	}
};
