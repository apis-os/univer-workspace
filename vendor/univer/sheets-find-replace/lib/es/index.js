	...
	*/
	...
	_getReplacedCellData(match, worksheet, shouldReplaceFormula, findString, replaceString, replaceFlag) {
		...
		if (match.isFormula) {
			...
			return {
				f: currentContent.f.replace(regexp.createLiteralRegExp(findString, replaceFlag), replaceString),
				v: null
			};
		}
		...
		return { v: currentContent.v.toString().replace(regexp.createLiteralRegExp(findString, replaceFlag), replaceString) };
	}
};
