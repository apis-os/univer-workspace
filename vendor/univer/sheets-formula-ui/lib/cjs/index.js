...
let RefSelectionsRenderService = class RefSelectionsRenderService extends _univerjs_sheets_ui.BaseSelectionRenderService {
	...
	*/
	_onPointerDown(evt, _zIndex = 0, rangeType = _univerjs_core.RANGE_TYPE.NORMAL, viewport, scrollTimerType = _univerjs_engine_render.ScrollTimerType.ALL) {
		...
		for (let i = 0; i < selectionControls.length; i++) if (i !== activeControlIndex && selectionControls[i].model.currentCell) selectionControls[i].clearHighlight();
		this._selectionMoveStart$.next(this.getSelectionDataWithStyle());
		if (expandByShiftKey && activeSelectionControl && isSelectionRangeChanged(rangeBeforePointerDown, activeSelectionControl.model)) this._selectionMoving$.next(this.getSelectionDataWithStyle());
...
function shouldAddFormulaReference(dataStream, index) {
	const char = dataStream[index - 1];
	const nextChar = dataStream[index];
	return Boolean(char && ((0, _univerjs_engine_formula.matchRefDrawToken)(char) || char === "!") && (!nextChar || (0, _univerjs_engine_formula.isFormulaLexerToken)(nextChar) && nextChar !== _univerjs_engine_formula.matchToken.OPEN_BRACKET));
}
function useFormulaSelecting(opts) {
	...
	const calculateSelectingType = (0, _univerjs_ui.useEvent)(() => {
		...
		const focusingNode = (lexerTreeBuilder.sequenceNodesBuilder(dataStream) ?? []).map((node) => {
			...
			return node;
		}).find((node) => typeof node === "object" && node.nodeType === _univerjs_engine_formula.sequenceNodeType.REFERENCE && index === node.endIndex + 2);
...
function calcHighlightRanges(opts) {
	...
	const getPrimaryForRange = (range, fallbackIndex) => {
		...
		if (isSameSelectionRange(currentSelection === null || currentSelection === void 0 ? void 0 : currentSelection.range, range)) {
			matchedCurrentSelectionIndexes.add(fallbackIndex);
			return currentSelection.primary;
		}
		const matchedIndex = currentSelections.findIndex((selection, index) => !matchedCurrentSelectionIndexes.has(index) && isSameSelectionRange(selection.range, range));
		if (matchedIndex !== -1) {
...
function isFormulaEditorInteractionOwner(focusEditorId, editorId, options) {
	...
	return editorId === (options.formulaBarEditorId ?? _univerjs_core.DOCS_FORMULA_BAR_EDITOR_UNIT_ID_KEY) && (focusEditorId === (options.normalEditorId ?? _univerjs_core.DOCS_NORMAL_EDITOR_UNIT_ID_KEY) || Boolean(options.allowMissingFocus && !focusEditorId));
}
...
const useRefactorEffect = (isNeed, selecting, unitId, editorId, disableContextMenu = true, keepRefSelectionsEnabled = false) => {
	const renderManagerService = (0, _univerjs_ui.useDependency)(_univerjs_engine_render.IRenderManagerService);
	const contextService = (0, _univerjs_ui.useDependency)(_univerjs_core.IContextService);
	...
	(0, react.useLayoutEffect)(() => {
		if (isNeed && Boolean(selecting)) {
			...
			const subscription = keepRefSelectionsEnabled ? contextService.subscribeContextValue$(_univerjs_sheets.REF_SELECTIONS_ENABLED).subscribe((enabled) => {
				if (!enabled) queueMicrotask(() => {
					if (active) contextService.setContextValue(_univerjs_sheets.REF_SELECTIONS_ENABLED, true);
				});
			}) : null;
			return () => {
				active = false;
...
function getSelectionsForFormulaRefUpdate(selections, updatingRefIndex, isCtrlAddMode) {
	...
	if (activeSelection) orderedSelections.splice(updatingRefIndex, 0, activeSelection);
	return {
		orderedSelections,
		insertedSelection
	};
...
const useSheetSelectionChange = (isNeed, isFocus, isSelectingRef, unitId, subUnitId, getRefSelections, isSupportAcrossSheet, listenSelectionSet, editor, handleRangeChange = () => {}, allowMissingEditorFocus = false) => {
	const renderManagerService = (0, _univerjs_ui.useDependency)(_univerjs_engine_render.IRenderManagerService);
	const univerInstanceService = (0, _univerjs_ui.useDependency)(_univerjs_core.IUniverInstanceService);
	const commandService = (0, _univerjs_ui.useDependency)(_univerjs_core.ICommandService);
	const contextService = (0, _univerjs_ui.useDependency)(_univerjs_core.IContextService);
	...
	const onSelectionsChange = (0, _univerjs_ui.useEvent)((selections, isEnd, isCtrlAddMode) => {
		if (!editor || !isFormulaEditorInteractionOwner(editorService.getFocusId(), editor.getEditorId(), {
			fxBarFocused: contextService.getContextValue(_univerjs_core.FOCUSING_FX_BAR_EDITOR),
		...
		if (isSelectingRef.current === 1 || (0, _univerjs_engine_formula.isFormulaReferenceAddingContext)(sequenceNodes, offset) || (0, _univerjs_engine_formula.isFormulaReferenceAddingTextContext)(formulaText, offset)) if (offset !== 0) {
			...
			const newTokens = sequenceNodes.map((item) => {
				...
				if (item.nodeType === _univerjs_engine_formula.sequenceNodeType.REFERENCE) {
					...
					if (insertedSelection && refIndex === updatingRefIndex) return `${refRangeText},${getRefRangeText(insertedSelection)}`;
					return refRangeText;
				}
				return item.token;
			});
	...
	(0, react.useEffect)(() => {
		if (refSelectionsRenderService && isNeed) {
			const isInteractionOwner = () => Boolean(editor && isFormulaEditorInteractionOwner(editorService.getFocusId(), editor.getEditorId(), {
				fxBarFocused: contextService.getContextValue(_univerjs_core.FOCUSING_FX_BAR_EDITOR),
				allowMissingFocus: allowMissingEditorFocus
			}));
...
//#endregion
//#region src/views/formula-editor/mobile-function-panel/MobileFunctionPanel.tsx
...
function readRecentFunctions() {
	try {
		var _globalThis$localStor;
		return JSON.parse(((_globalThis$localStor = globalThis.localStorage) === null || _globalThis$localStor === void 0 ? void 0 : _globalThis$localStor.getItem(RECENT_FUNCTIONS_KEY)) ?? "[]");
	} catch {
		return [];
	}
}
function rememberFunction(name) {
	try {
		var _globalThis$localStor2;
		const recent = [name, ...readRecentFunctions().filter((item) => item !== name)].slice(0, 8);
		(_globalThis$localStor2 = globalThis.localStorage) === null || _globalThis$localStor2 === void 0 || _globalThis$localStor2.setItem(RECENT_FUNCTIONS_KEY, JSON.stringify(recent));
	} catch {}
}
function MobileFunctionPanel(props) {
	const { open, onClose, onInsert } = props;
	const descriptionService = (0, _univerjs_ui.useDependency)(_univerjs_engine_formula.IDescriptionService);
	const localeService = (0, _univerjs_ui.useDependency)(_univerjs_core.LocaleService);
	const copy = {
		title: localeService.t("sheets-formula-ui.mobileFunction.title"),
		search: localeService.t("sheets-formula-ui.moreFunctions.searchFunctionPlaceholder"),
		recommended: localeService.t("sheets-formula-ui.mobileFunction.recommended"),
		recent: localeService.t("sheets-formula-ui.mobileFunction.recent"),
		all: localeService.t("sheets-formula-ui.moreFunctions.allFunctions"),
		details: localeService.t("sheets-formula-ui.mobileFunction.details"),
		back: localeService.t("sheets-formula-ui.moreFunctions.prev"),
		insert: localeService.t("sheets-formula-ui.mobileFunction.insert"),
		empty: localeService.t("sheets-formula-ui.mobileFunction.empty"),
		syntax: localeService.t("sheets-formula-ui.moreFunctions.syntax"),
		close: localeService.t("sheets-formula-ui.mobileFunction.close")
	};
	const [query, setQuery] = (0, react.useState)("");
	const [category, setCategory] = (0, react.useState)("recommended");
	const [details, setDetails] = (0, react.useState)(null);
	const [recentVersion, setRecentVersion] = (0, react.useState)(0);
	(0, react.useEffect)(() => {
		if (!open) return;
		setQuery("");
	...
	const categories = (0, react.useMemo)(() => {
		...
		return [
			...
			...getFunctionTypeValues(localeService, false).map((item) => ({
				label: item.label,
				value: Number(item.value)
			}))
		].filter((item) => typeof item.value !== "number" || item.value !== _univerjs_engine_formula.FunctionType.DefinedName && item.value !== _univerjs_engine_formula.FunctionType.Table && descriptionService.getSearchListByType(item.value).length > 0);
	}, [
		copy.all,
		copy.recent,
		copy.recommended,
	...
	const functions = (0, react.useMemo)(() => {
		if (!open) return [];
		if (query.trim()) return descriptionService.getSearchListByName(query).slice(0, 60);
		if (category === "recommended") return RECOMMENDED_FUNCTIONS.map((name) => descriptionService.getFunctionInfo(name)).filter((item) => Boolean(item)).map((item) => ({
			name: item.functionName,
			desc: item.abstract
		}));
		if (category === "recent") return readRecentFunctions().map((name) => descriptionService.getFunctionInfo(name)).filter((item) => Boolean(item)).map((item) => ({
			name: item.functionName,
			desc: item.abstract
		}));
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		...
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
			...
			className: "univer-absolute univer-inset-x-0 univer-bottom-0 univer-flex univer-h-[80dvh] univer-flex-col univer-overflow-hidden univer-rounded-t-2xl univer-bg-gray-0 univer-shadow-lg dark:!univer-bg-gray-900",
			children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: "univer-flex univer-justify-center univer-pb-2 univer-pt-3",
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { className: "univer-h-1 univer-w-10 univer-rounded-full univer-bg-gray-300 dark:!univer-bg-gray-600" })
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
					className: "univer-flex univer-h-12 univer-items-center univer-gap-2 univer-px-4",
					children: [
						details && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: "univer-h-10 univer-appearance-none univer-rounded-lg univer-border-0 univer-bg-transparent univer-px-3 univer-text-sm univer-text-primary-600 univer-outline-none active:univer-bg-primary-50",
							onClick: () => setDetails(null),
							children: copy.back
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", {
							className: "univer-m-0 univer-flex-1 univer-text-base univer-font-semibold univer-text-gray-900 dark:!univer-text-gray-0",
							children: (details === null || details === void 0 ? void 0 : details.functionName) ?? copy.title
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							"aria-label": copy.close,
							className: "univer-size-10 univer-appearance-none univer-rounded-lg univer-border-0 univer-bg-transparent univer-text-xl univer-text-gray-600 univer-outline-none active:univer-bg-gray-100 dark:!univer-text-gray-200 dark:active:!univer-bg-gray-700",
							onClick: onClose,
							children: "×"
						})
					]
				}),
				details ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: (0, _univerjs_design.clsx)("univer-flex-1 univer-overflow-y-auto univer-px-4 univer-pb-4", _univerjs_design.scrollbarClassName),
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "univer-rounded-xl univer-bg-gray-50 univer-p-4 dark:!univer-bg-gray-800",
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: "univer-m-0 univer-text-sm univer-leading-6 univer-text-gray-700 dark:!univer-text-gray-200",
								children: details.description
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: "univer-mt-4 univer-text-xs univer-text-gray-500 dark:!univer-text-gray-300",
								children: copy.syntax
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", {
								className: "univer-mt-2 univer-block univer-break-words univer-rounded-lg univer-bg-gray-0 univer-p-3 univer-text-sm dark:!univer-bg-gray-900",
								children: `${details.functionName}(${details.functionParameter.map((item) => item.name).join(", ")})`
							})
						]
					}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						type: "button",
						"data-u-comp": "mobile-formula-insert",
						className: "univer-mt-4 univer-h-12 univer-w-full univer-appearance-none univer-rounded-xl univer-border-0 univer-bg-primary-600 univer-text-sm univer-font-medium univer-text-white univer-outline-none active:univer-bg-primary-700",
						onClick: () => handleInsert(details.functionName),
						children: copy.insert
					})]
				}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "univer-px-4 univer-pb-3",
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							value: query,
							"aria-label": copy.search,
							placeholder: copy.search,
							className: "univer-box-border univer-h-11 univer-w-full univer-rounded-xl univer-border univer-border-solid univer-border-gray-200 univer-bg-gray-50 univer-px-4 univer-text-base univer-outline-none focus:univer-border-primary-500 dark:!univer-border-gray-700 dark:!univer-bg-gray-800 dark:!univer-text-gray-0",
							onChange: (event) => setQuery(event.target.value)
						})
					}),
					!query && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "univer-flex univer-shrink-0 univer-gap-2 univer-overflow-x-auto univer-px-4 univer-pb-3",
						style: { scrollbarWidth: "none" },
						children: categories.map((item) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: (0, _univerjs_design.clsx)("univer-h-10 univer-shrink-0 univer-appearance-none univer-rounded-xl univer-border-0 univer-px-4 univer-text-sm univer-outline-none active:univer-scale-[0.98]", {
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: (0, _univerjs_design.clsx)("univer-flex-1 univer-overflow-y-auto univer-px-4 univer-pb-4", _univerjs_design.scrollbarClassName),
						children: [functions.length === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "univer-py-12 univer-text-center univer-text-sm univer-text-gray-400",
							children: copy.empty
						}), functions.map((item) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "univer-mb-2 univer-flex univer-min-h-14 univer-items-center univer-rounded-xl univer-bg-gray-50 univer-pl-4 dark:!univer-bg-gray-800",
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
								type: "button",
								className: "univer-min-w-0 univer-flex-1 univer-appearance-none univer-border-0 univer-bg-transparent univer-py-3 univer-text-left univer-outline-none active:univer-opacity-60",
								onClick: () => handleInsert(item.name),
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "univer-block univer-text-sm univer-font-medium univer-text-gray-900 dark:!univer-text-gray-0",
									children: item.name
								}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "univer-mt-1 univer-block univer-truncate univer-text-xs univer-text-gray-500 dark:!univer-text-gray-300",
									children: item.desc
								})]
							}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "univer-h-12 univer-shrink-0 univer-appearance-none univer-border-0 univer-bg-transparent univer-px-4 univer-text-sm univer-text-primary-600 univer-outline-none active:univer-bg-primary-50",
...
function SearchFunctionFactory(props, ref) {
	const { isFocus, sequenceNodes, onSelect, editor, onClose = _univerjs_core.noop, mobile = false } = props;
	const editorId = editor.getEditorId();
	const shortcutService = (0, _univerjs_ui.useDependency)(_univerjs_ui.IShortcutService);
	const commandService = (0, _univerjs_ui.useDependency)(_univerjs_core.ICommandService);
	...
	return searchList.length > 0 && visible && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_univerjs_ui.RectPopup, {
		...
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", {
			...
			className: (0, _univerjs_design.clsx)("univer-m-0 univer-box-border univer-list-none univer-overflow-y-auto univer-rounded-lg univer-bg-gray-0 univer-p-2 univer-leading-5 univer-shadow-md univer-outline-none dark:!univer-bg-gray-900", mobile ? "univer-max-h-[38dvh] univer-w-[calc(100vw-16px)]" : "univer-max-h-[400px] univer-w-[250px]", _univerjs_design.borderClassName, _univerjs_design.scrollbarClassName),
			children: searchList.map((item, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
				className: (0, _univerjs_design.clsx)("univer-box-border univer-cursor-pointer univer-rounded univer-px-2 univer-py-1 univer-text-gray-900 univer-transition-colors dark:!univer-text-gray-0", { "univer-flex univer-min-h-12 univer-flex-col univer-justify-center univer-px-3 univer-py-2": mobile }, { "univer-bg-gray-200 dark:!univer-bg-gray-600": active === index }),
...
function buildFormulaFunctionInsertion(formulaText, selection, functionName) {
	...
	const startOffset = Math.max(1, Math.min(text.length, ((selection === null || selection === void 0 ? void 0 : selection.startOffset) ?? text.length) + selectionAdjustment));
	const endOffset = Math.max(startOffset, Math.min(text.length, ((selection === null || selection === void 0 ? void 0 : selection.endOffset) ?? startOffset) + selectionAdjustment));
	return {
		text: `${text.slice(0, startOffset)}${functionName}()${text.slice(endOffset)}`,
		caretOffset: startOffset + functionName.length + 1
	};
}
function buildFormulaOperatorInsertion(formulaText, selection, value) {
	const normalizedText = formulaText.replace(/\r?\n$/, "");
	const text = normalizedText.startsWith("=") ? normalizedText : `=${normalizedText}`;
	const selectionAdjustment = normalizedText.startsWith("=") ? 0 : 1;
	const startOffset = Math.max(1, Math.min(text.length, ((selection === null || selection === void 0 ? void 0 : selection.startOffset) ?? text.length) + selectionAdjustment));
...
const FormulaEditor = (0, react.forwardRef)((props, ref) => {
	...
	const { errorText, initValue, unitId, subUnitId, isFocus: _isFocus = true, isSupportAcrossSheet = false, onFocus = _univerjs_core.noop, onBlur = _univerjs_core.noop, onChange: propOnChange, onVerify, className, editorId: propEditorId, moveCursor = true, onFormulaSelectingChange: propOnFormulaSelectingChange, keyboardEventConfig, onMoveInEditor, resetSelectionOnBlur = true, autoScrollbar = true, isSingle = true, disableSelectionOnClick = false, autofocus = true, disableContextMenu, style, borderless = false, canvasStyle, mobile = false, mobileFxRequest, mobileFunctionPanelRequest, mobileOperatorRequest, onMobileFormulaActiveChange: propOnMobileFormulaActiveChange } = props;
	const editorService = (0, _univerjs_ui.useDependency)(_univerjs_docs_ui.IEditorService);
	const commandService = (0, _univerjs_ui.useDependency)(_univerjs_core.ICommandService);
	const injector = (0, _univerjs_ui.useDependency)(_univerjs_core.Injector);
	...
	const onMobileFormulaActiveChange = (0, _univerjs_ui.useEvent)(propOnMobileFormulaActiveChange);
	...
	const [mobileFunctionPanelOpen, setMobileFunctionPanelOpen] = (0, react.useState)(false);
	const { isSelecting, isSelectingRef } = useFormulaSelecting({
		...
		resetSignal: mobile ? mobileFxRequest : void 0
	});
	...
	(0, react.useEffect)(() => {
		if (mobile) onMobileFormulaActiveChange === null || onMobileFormulaActiveChange === void 0 || onMobileFormulaActiveChange(formulaText.replace(/\r?\n$/, "").startsWith("="));
	}, [
		formulaText,
		mobile,
		onMobileFormulaActiveChange
	]);
	(0, react.useEffect)(() => {
		if (!isFocus || !editor) return;
	...
	const handledMobileFunctionPanelRequestRef = (0, react.useRef)(mobileFunctionPanelRequest);
	(0, react.useEffect)(() => {
		if (!mobile || mobileFunctionPanelRequest === void 0 || mobileFunctionPanelRequest === handledMobileFunctionPanelRequestRef.current) return;
	...
	(0, _univerjs_docs_ui.useKeyboardEvent)(isFocus, (0, react.useMemo)(() => (0, _univerjs_docs_ui.createEditorUndoRedoKeyboardConfig)({
		...
		keyCodes: keyboardEventConfig === null || keyboardEventConfig === void 0 ? void 0 : keyboardEventConfig.keyCodes,
		handler: keyboardEventConfig === null || keyboardEventConfig === void 0 ? void 0 : keyboardEventConfig.handler
	}), [
		commandService,
		editorId,
		keyboardEventConfig,
	...
	useSheetSelectionChange(isFocus && Boolean(isSelecting), isFocus, isSelectingRef, unitId, subUnitId, getRefSelections, isSupportAcrossSheet, Boolean(selectingMode), editor, handleSelectionChange, mobile);
	...
	const applyMobileInsertion = (0, _univerjs_ui.useEvent)((result) => {
		if (!editor) return;
		const selection = {
		...
		const counterpartEditorId = editorId === _univerjs_core.DOCS_FORMULA_BAR_EDITOR_UNIT_ID_KEY ? _univerjs_core.DOCS_NORMAL_EDITOR_UNIT_ID_KEY : editorId === _univerjs_core.DOCS_NORMAL_EDITOR_UNIT_ID_KEY ? _univerjs_core.DOCS_FORMULA_BAR_EDITOR_UNIT_ID_KEY : null;
		if (counterpartEditorId) {
			const counterpartEditor = editorService.getEditor(counterpartEditorId);
			counterpartEditor === null || counterpartEditor === void 0 || counterpartEditor.replaceText(result.text, false);
			counterpartEditor === null || counterpartEditor === void 0 || counterpartEditor.setSelectionRanges([selection], false);
		}
		highlight(result.text, false, true, [selection]);
		requestAnimationFrame(() => focus());
	});
	const handledMobileOperatorRequestRef = (0, react.useRef)(mobileOperatorRequest === null || mobileOperatorRequest === void 0 ? void 0 : mobileOperatorRequest.id);
	(0, react.useEffect)(() => {
		var _editor$getDocumentDa2, _editor$getSelectionR2;
		if (!mobile || !editor || !(mobileOperatorRequest === null || mobileOperatorRequest === void 0 ? void 0 : mobileOperatorRequest.value) || mobileOperatorRequest.id === handledMobileOperatorRequestRef.current) return;
		handledMobileOperatorRequestRef.current = mobileOperatorRequest.id;
		const currentText = _univerjs_core.BuildTextUtils.transform.getPlainText(((_editor$getDocumentDa2 = editor.getDocumentData().body) === null || _editor$getDocumentDa2 === void 0 ? void 0 : _editor$getDocumentDa2.dataStream) ?? "");
		applyMobileInsertion(buildFormulaOperatorInsertion(currentText, (_editor$getSelectionR2 = editor.getSelectionRanges()) === null || _editor$getSelectionR2 === void 0 ? void 0 : _editor$getSelectionR2[0], mobileOperatorRequest.value));
	}, [
		applyMobileInsertion,
		editor,
		mobile,
		mobileOperatorRequest
	]);
	const handleMobileFunctionInsert = (functionName) => {
		var _editor$getDocumentDa3, _editor$getSelectionR3;
		if (!editor) return;
		const result = buildFormulaFunctionInsertion(_univerjs_core.BuildTextUtils.transform.getPlainText(((_editor$getDocumentDa3 = editor.getDocumentData().body) === null || _editor$getDocumentDa3 === void 0 ? void 0 : _editor$getDocumentDa3.dataStream) ?? ""), (_editor$getSelectionR3 = editor.getSelectionRanges()) === null || _editor$getSelectionR3 === void 0 ? void 0 : _editor$getSelectionR3[0], functionName);
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [
			...
			!mobile && functionScreenTips && editor && formulaWithoutEqualSymbol !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(HelpFunction, {
				editor,
				isFocus,
				formulaText,
				onClose: () => focus()
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)(MobileFunctionPanel, {
...
//#endregion
//#region src/views/range-selector/MobileRangeSelectorDialog.tsx
function MobileRangeSelectorDialog(props) {
	const { visible, snap, title, cancelText, confirmText, children, onSnapChange, onClose, onConfirm } = props;
	if (!visible) return null;
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		className: "univer-pointer-events-none univer-visible univer-fixed univer-inset-0 univer-z-[1300]",
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_univerjs_ui.MobileDrawer, {
			...
			header: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("header", {
				className: "univer-flex univer-h-12 univer-flex-1 univer-items-center univer-px-4 univer-text-base univer-font-semibold",
				children: title
			}),
			footer: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("footer", {
				className: "univer-box-border univer-shrink-0 univer-p-4",
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(_univerjs_design.ActionRow, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_univerjs_design.Button, {
					onClick: onClose,
					children: cancelText
...
//#endregion
//#region src/views/range-selector/utils/verify-range.ts
/**
...
function RangeSelectorDialog(props) {
	const { visible, initialValue, unitId, subUnitId, maxRangeCount = Infinity, supportAcrossSheet, keepSheetReference, onConfirm, onClose, onShowBySelection, mobile = false } = props;
	const localeService = (0, _univerjs_ui.useDependency)(_univerjs_core.LocaleService);
	const lexerTreeBuilder = (0, _univerjs_ui.useDependency)(_univerjs_engine_formula.LexerTreeBuilder);
	...
	const rangeInputs = (mobileLayout = false) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		ref: scrollbarRef,
		className: (0, _univerjs_design.clsx)("univer-overflow-y-auto", _univerjs_design.scrollbarClassName, mobileLayout ? "univer-max-h-[22dvh]" : "-univer-mx-6 univer-max-h-60 univer-px-6"),
		children: [ranges.map((text, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			className: (0, _univerjs_design.clsx)("univer-mb-2 univer-flex univer-items-center", mobileLayout ? "univer-gap-2" : "univer-gap-4"),
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_univerjs_design.Input, {
				className: (0, _univerjs_design.clsx)("univer-box-border univer-h-10 univer-w-full", { "univer-border-primary-600": focusIndex === index }),
				placeholder: localeService.t("sheets-formula-ui.rangeSelector.placeHolder"),
				onFocus: () => setFocusIndex(index),
				value: text,
				onChange: (value) => handleRangeInput(index, value)
			}), ranges.length > 1 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
				type: "button",
				className: "univer-flex univer-size-10 univer-shrink-0 univer-items-center univer-justify-center univer-rounded-lg univer-border-0 univer-bg-transparent univer-text-gray-600 active:univer-bg-gray-100 dark:!univer-text-gray-300 dark:active:!univer-bg-gray-700",
				onClick: () => handleRangeRemove(index),
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_univerjs_icons.DeleteIcon, { className: "univer-cursor-pointer" })
			})]
		}, index)), ranges.length < maxRangeCount && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(_univerjs_design.Button, {
			className: mobileLayout ? "univer-h-10 univer-w-full" : void 0,
			variant: "link",
	...
	if (mobile) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(MobileRangeSelectorDialog, {
		visible,
		snap: drawerSnap,
		title: localeService.t("sheets-formula-ui.rangeSelector.title"),
		cancelText: localeService.t("sheets-formula-ui.rangeSelector.cancel"),
		confirmText: localeService.t("sheets-formula-ui.rangeSelector.confirm"),
		onSnapChange: setDrawerSnap,
		onClose,
		onConfirm: confirmRanges,
		children: rangeInputs(true)
	});
...
function RangeSelector(props) {
	...
	const { mobile = false } = (0, react.useContext)(_univerjs_design.ConfigContext);
	const { onVerify, selectorRef, unitId, subUnitId, maxRangeCount, supportAcrossSheet, keepSheetReference, autoFocus, onChange, onRangeSelectorDialogVisibleChange, onClickOutside, onFocusChange, forceShowDialogWhenSelectionChanged, hideEditor, resetRange, onClose } = props;
...
let ComponentsController = class ComponentsController extends _univerjs_core.Disposable {
	...
	_registerComponents() {
		...
		this.disposeWithMe(this._componentManager.register(_univerjs_sheets_ui.RANGE_SELECTOR_COMPONENT_KEY, RangeSelector));
		this.disposeWithMe(this._componentManager.register(_univerjs_sheets_ui.EMBEDDING_FORMULA_EDITOR_COMPONENT_KEY, FormulaEditor));
	}
};
...
const GlobalRangeSelector = () => {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(RangeSelector, {
		...
		maxRangeCount: current === null || current === void 0 ? void 0 : current.maxRangeCount,
		hideEditor: true,
		selectorRef: instance,
		onClose: () => current === null || current === void 0 ? void 0 : current.callback([]),
		onChange: (_, value) => {
