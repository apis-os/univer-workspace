...
import { ImageFormulaCellInterceptorController, InsertFunctionCommand, PLUGIN_CONFIG_KEY_BASE, TriggerCalculationController, UniverSheetsFormulaPlugin } from "@univerjs/sheets-formula";
import { BuiltInUIPart, ComponentManager, IContextMenuService, IMenuManagerService, IShortcutService, ISidebarService, IUIPartsService, IconManager, KeyCode, MenuItemType, MetaKeys, MobileDrawer, ProgressBar, RectPopup, RibbonFormulasGroup, connectInjector, getMenuHiddenObservable, useDependency, useEvent, useObservable, useUpdateEffect } from "@univerjs/ui";
...
import { ActionRow, Button, ConfigContext, Dialog, Input, Select, Tooltip, borderClassName, borderLeftClassName, borderTopClassName, clsx, divideYClassName, scrollbarClassName } from "@univerjs/design";
...
function shouldAddFormulaReference(dataStream, index) {
	...
	return Boolean(char && (matchRefDrawToken(char) || char === "!") && (!nextChar || isFormulaLexerToken(nextChar) && nextChar !== matchToken.OPEN_BRACKET));
}
function useFormulaSelecting(opts) {
	...
	const calculateSelectingType = useEvent(() => {
		...
		const focusingNode = (lexerTreeBuilder.sequenceNodesBuilder(dataStream) ?? []).map((node) => {
			...
			return node;
		}).find((node) => typeof node === "object" && node.nodeType === sequenceNodeType.REFERENCE && index === node.endIndex + 2);
...
function isFormulaEditorInteractionOwner(focusEditorId, editorId, options) {
	...
	return editorId === (options.formulaBarEditorId ?? DOCS_FORMULA_BAR_EDITOR_UNIT_ID_KEY) && (focusEditorId === (options.normalEditorId ?? DOCS_NORMAL_EDITOR_UNIT_ID_KEY) || Boolean(options.allowMissingFocus && !focusEditorId));
}
...
const useRefactorEffect = (isNeed, selecting, unitId, editorId, disableContextMenu = true, keepRefSelectionsEnabled = false) => {
	...
	useLayoutEffect(() => {
		if (isNeed && Boolean(selecting)) {
			...
			const subscription = keepRefSelectionsEnabled ? contextService.subscribeContextValue$(REF_SELECTIONS_ENABLED).subscribe((enabled) => {
...
function MobileFunctionPanel(props) {
	...
	const categories = useMemo(() => {
		...
		].filter((item) => typeof item.value !== "number" || item.value !== FunctionType.DefinedName && item.value !== FunctionType.Table && descriptionService.getSearchListByType(item.value).length > 0);
	}, [
		copy.all,
		copy.recent,
		copy.recommended,
		...
		children: /* @__PURE__ */ jsxs("section", {
			...
			children: [
				/* @__PURE__ */ jsx("div", {
					...
					children: /* @__PURE__ */ jsx("div", { className: "univer-h-1 univer-w-10 univer-rounded-full univer-bg-gray-300 dark:!univer-bg-gray-600" })
				}),
				...
				details ? /* @__PURE__ */ jsxs("div", {
					className: clsx("univer-flex-1 univer-overflow-y-auto univer-px-4 univer-pb-4", scrollbarClassName),
					children: [/* @__PURE__ */ jsxs("div", {
					...
					!query && /* @__PURE__ */ jsx("div", {
						...
						children: categories.map((item) => /* @__PURE__ */ jsx("button", {
							type: "button",
							className: clsx("univer-h-10 univer-shrink-0 univer-appearance-none univer-rounded-xl univer-border-0 univer-px-4 univer-text-sm univer-outline-none active:univer-scale-[0.98]", {
					...
					/* @__PURE__ */ jsxs("div", {
						className: clsx("univer-flex-1 univer-overflow-y-auto univer-px-4 univer-pb-4", scrollbarClassName),
						children: [functions.length === 0 && /* @__PURE__ */ jsx("div", {
							className: "univer-py-12 univer-text-center univer-text-sm univer-text-gray-400",
							children: copy.empty
						}), functions.map((item) => /* @__PURE__ */ jsxs("div", {
...
const FormulaEditor = forwardRef((props, ref) => {
	...
	const applyMobileInsertion = useEvent((result) => {
		if (!editor) return;
		const selection = {
		...
		const counterpartEditorId = editorId === DOCS_FORMULA_BAR_EDITOR_UNIT_ID_KEY ? DOCS_NORMAL_EDITOR_UNIT_ID_KEY : editorId === DOCS_NORMAL_EDITOR_UNIT_ID_KEY ? DOCS_FORMULA_BAR_EDITOR_UNIT_ID_KEY : null;
	...
	const handledMobileOperatorRequestRef = useRef(mobileOperatorRequest === null || mobileOperatorRequest === void 0 ? void 0 : mobileOperatorRequest.id);
	useEffect(() => {
		...
		const currentText = BuildTextUtils.transform.getPlainText(((_editor$getDocumentDa2 = editor.getDocumentData().body) === null || _editor$getDocumentDa2 === void 0 ? void 0 : _editor$getDocumentDa2.dataStream) ?? "");
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [
			...
			!mobile && functionScreenTips && editor && formulaWithoutEqualSymbol !== "" && /* @__PURE__ */ jsx(HelpFunction, {
				editor,
				isFocus,
				formulaText,
				onClose: () => focus()
			...
			/* @__PURE__ */ jsx(MobileFunctionPanel, {
...
function MobileRangeSelectorDialog(props) {
	...
	return /* @__PURE__ */ jsx("div", {
		...
		children: /* @__PURE__ */ jsx(MobileDrawer, {
			...
			footer: /* @__PURE__ */ jsx("footer", {
				...
				children: /* @__PURE__ */ jsxs(ActionRow, { children: [/* @__PURE__ */ jsx(Button, {
					onClick: onClose,
					children: cancelText
				}), /* @__PURE__ */ jsx(Button, {
					variant: "primary",
...
function RangeSelectorDialog(props) {
	...
	const rangeInputs = (mobileLayout = false) => /* @__PURE__ */ jsxs("div", {
		ref: scrollbarRef,
		className: clsx("univer-overflow-y-auto", scrollbarClassName, mobileLayout ? "univer-max-h-[22dvh]" : "-univer-mx-6 univer-max-h-60 univer-px-6"),
		children: [ranges.map((text, index) => /* @__PURE__ */ jsxs("div", {
			className: clsx("univer-mb-2 univer-flex univer-items-center", mobileLayout ? "univer-gap-2" : "univer-gap-4"),
			children: [/* @__PURE__ */ jsx(Input, {
				className: clsx("univer-box-border univer-h-10 univer-w-full", { "univer-border-primary-600": focusIndex === index }),
				placeholder: localeService.t("sheets-formula-ui.rangeSelector.placeHolder"),
				onFocus: () => setFocusIndex(index),
				value: text,
				onChange: (value) => handleRangeInput(index, value)
			}), ranges.length > 1 && /* @__PURE__ */ jsx("button", {
				type: "button",
				...
				children: /* @__PURE__ */ jsx(DeleteIcon, { className: "univer-cursor-pointer" })
			})]
		}, index)), ranges.length < maxRangeCount && /* @__PURE__ */ jsxs(Button, {
	...
	if (mobile) return /* @__PURE__ */ jsx(MobileRangeSelectorDialog, {
		visible,
		snap: drawerSnap,
