...
let UpdateFormulaController = class UpdateFormulaController extends _univerjs_core.Disposable {
	...
	_handleInsertSheetMutation(params) {
		...
		const cellMatrix = new _univerjs_core.ObjectMatrix(cellData);
	...
	_initialConversion() {
		this._activeDirtyManagerService.register(_univerjs_sheets.SetRangeValuesMutation.id, {
			...
			shouldTrigger: (command, options) => {
				const params = command.params;
				return !(options === null || options === void 0 ? void 0 : options.onlyLocal) && params.trigger !== _univerjs_sheets.SetStyleCommand.id && params.trigger !== _univerjs_sheets.SetBorderCommand.id && params.trigger !== _univerjs_sheets.ClearSelectionFormatCommand.id;
			},
...
* limitations under the License.
*/
let SheetFormulaCalculationResultApplyController = class SheetFormulaCalculationResultApplyController extends _univerjs_core.Disposable {
...
let UniverSheetsFormulaPlugin = class UniverSheetsFormulaPlugin extends _univerjs_core.Plugin {
	...
	onReady() {
		...
		if ((0, _univerjs_core.isNodeEnv)()) (0, _univerjs_core.touchDependencies)(this._injector, [[TriggerCalculationController]]);
	}
	onRendered() {
