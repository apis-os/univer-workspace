...
_univerjs_sheets_facade.FWorkbook.extend(FWorkbookHyperlinkMixin);
...
var FRangeSheetsHyperlinkMixin = class extends _univerjs_sheets_facade.FRange {
	...
	getHyperLinks() {
		...
		this.forEach((row, column, cell) => {
			var _cell$p$body;
			if (!cell.p) return;
			const ranges = ((_cell$p$body = cell.p.body) === null || _cell$p$body === void 0 || (_cell$p$body = _cell$p$body.customRanges) === null || _cell$p$body === void 0 ? void 0 : _cell$p$body.filter((range) => range.rangeType === _univerjs_core.CustomRangeType.HYPERLINK)) ?? [];
			if (ranges.length > 0) {
				var _cell$p;
				...
				const url = (properties === null || properties === void 0 ? void 0 : properties.url) ?? "";
				const label = (dataStream === null || dataStream === void 0 ? void 0 : dataStream.slice(startIndex, endIndex + 1).replaceAll(_univerjs_core.DataStreamTreeTokenType.CUSTOM_RANGE_START, "").replaceAll(_univerjs_core.DataStreamTreeTokenType.CUSTOM_RANGE_END, "")) ?? "";
				hyperlinks.push({
					id: `${rangeId}`,
					row,
					column,
...
var FSheetsHyperlinkEventNameMixin = class extends _univerjs_core_facade.FEventName {
...
_univerjs_core_facade.FEventName.extend(FSheetsHyperlinkEventNameMixin);

//#endregion
//#region src/facade/f-univer.ts
var FUniverSheetsHyperlinkMixin = class extends _univerjs_core_facade.FUniver {
	/**
	* @ignore
	*/
...
_univerjs_core_facade.FUniver.extend(FUniverSheetsHyperlinkMixin);

//#endregion
exports.FWorkbookHyperlinkMixin = FWorkbookHyperlinkMixin;