...
import { IDefinedNamesService, UniverFormulaEnginePlugin, deserializeRangeWithSheet, serializeRange, serializeRangeWithSheet } from "@univerjs/engine-formula";
...
let HyperLinkModel = class HyperLinkModel extends Disposable {
	...
	getHyperLinkByLocationSync(unitId, subUnitId, row, column) {
		var _workbook$getSheetByS, _cell$p;
		const { matrix } = this._ensureMap(unitId, subUnitId);
		...
		const cellValueStr = ((cell === null || cell === void 0 ? void 0 : cell.v) ?? (cell === null || cell === void 0 || (_cell$p = cell.p) === null || _cell$p === void 0 || (_cell$p = _cell$p.body) === null || _cell$p === void 0 ? void 0 : _cell$p.dataStream.slice(0, -2)) ?? "").toString();
...
const UpdateRichHyperLinkCommand = {
	...
	handler: (accessor, params) => {
		var _doc$getBody, _oldBody$textRuns2;
		if (!params) return false;
...
let SheetsHyperLinkParserService = class SheetsHyperLinkParserService {
	...
	_getURLName(params) {
		...
		const sheetName = (sheet === null || sheet === void 0 ? void 0 : sheet.getName()) ?? "";
		if (range) {
