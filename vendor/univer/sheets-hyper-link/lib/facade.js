...
var FRangeSheetsHyperlinkMixin = class extends FRange {
	...
	getHyperLinks() {
		...
		this.forEach((row, column, cell) => {
			var _cell$p$body;
			if (!cell.p) return;
			const ranges = ((_cell$p$body = cell.p.body) === null || _cell$p$body === void 0 || (_cell$p$body = _cell$p$body.customRanges) === null || _cell$p$body === void 0 ? void 0 : _cell$p$body.filter((range) => range.rangeType === CustomRangeType.HYPERLINK)) ?? [];
			if (ranges.length > 0) {
				var _cell$p;
				...
				const label = (dataStream === null || dataStream === void 0 ? void 0 : dataStream.slice(startIndex, endIndex + 1).replaceAll(DataStreamTreeTokenType.CUSTOM_RANGE_START, "").replaceAll(DataStreamTreeTokenType.CUSTOM_RANGE_END, "")) ?? "";
				hyperlinks.push({
					id: `${rangeId}`,
					row,
					column,
