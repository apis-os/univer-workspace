...
const SetMobileNumfmtCommand = {
	...
	handler: async (accessor, params) => {
		...
		const type = pattern ? (0, _univerjs_sheets_numfmt.getPatternType)(pattern) : void 0;
		const values = [];
		selections.forEach((selection) => {
			_univerjs_core.Range.foreach(selection.range, (row, col) => values.push({
				row,
				col,
				pattern,
				type
		...
		return accessor.get(_univerjs_core.ICommandService).executeCommand(_univerjs_sheets_numfmt.SetNumfmtCommand.id, { values });
	}
};
...
function createMobileNumberFormatMenuConfig(currencySymbol) {
	...
	return {
		...
		quickOptions: [
			{
				label: "sheets-numfmt-ui.percent",
				commandId: _univerjs_sheets_numfmt.SetPercentCommand.id
			},
			{
				label: "sheets-numfmt-ui.currency",
				commandId: _univerjs_sheets_numfmt.SetCurrencyCommand.id
			},
			{
				label: "sheets-numfmt-ui.date",
				commandId: SetMobileNumfmtCommand.id,
			...
			},
			{
				label: "sheets-numfmt-ui.text",
				commandId: SetMobileNumfmtCommand.id,
				value: _univerjs_core.DEFAULT_TEXT_FORMAT_EXCEL
			}
		],
		decimalOptions: [{
			label: "sheets-numfmt-ui.subtractDecimal",
			commandId: _univerjs_sheets_numfmt.SubtractDecimalCommand.id
		}, {
			label: "sheets-numfmt-ui.addDecimal",
			commandId: _univerjs_sheets_numfmt.AddDecimalCommand.id
		}],
		detailOptions,
		customPatterns: [.../* @__PURE__ */ new Set([
			..._univerjs_sheets_numfmt.CURRENCYFORMAT.map((item) => item.suffix(currencySymbol)),
...
function FactoryOtherMenuItem(accessor) {
	...
	return {
		...
		mobileStyle: createMobileNumberFormatMenuConfig((0, _univerjs_sheets_numfmt.getCurrencySymbolByLocale)(regionService.getCurrentRegion()))
	};
}
...
function getTemporalType(pattern) {
	if (_univerjs_core.numfmt.getFormatDateInfo(pattern).isDuration) return "duration";
	const type = _univerjs_core.numfmt.getFormatInfo(pattern).type;
	return type === "date" || type === "datetime" || type === "time" ? type : null;
}
function trimFraction(value, locale) {
	var _numfmt$getLocale;
	const decimal = ((_numfmt$getLocale = _univerjs_core.numfmt.getLocale(locale)) === null || _numfmt$getLocale === void 0 ? void 0 : _numfmt$getLocale.decimal) ?? ".";
...
function serializeDuration(serial, locale) {
	...
	const fraction = milliseconds ? `${((_numfmt$getLocale2 = _univerjs_core.numfmt.getLocale(locale)) === null || _numfmt$getLocale2 === void 0 ? void 0 : _numfmt$getLocale2.decimal) ?? "."}${String(milliseconds).padStart(3, "0").replace(/0+$/, "")}` : "";
...
function serializeTemporalEditorValue(options) {
	...
	const parts = (0, _univerjs_core.excelSerialToDateTimeParts)(serial, { dateSystem });
	if (!parts) return null;
	const hasFraction = Math.round(parts.fractionalSecond * 1e3) > 0;
	const canonicalPattern = _univerjs_core.numfmt.getDateTimeEditPattern(pattern, serial);
	if (!canonicalPattern) return null;
	const value = _univerjs_core.numfmt.format(canonicalPattern, serial, {
		locale,
		dateSystem,
		throws: false
	});
...
function parseDuration(content, locale) {
	...
	const decimal = ((_numfmt$getLocale3 = _univerjs_core.numfmt.getLocale(locale)) === null || _numfmt$getLocale3 === void 0 ? void 0 : _numfmt$getLocale3.decimal) ?? ".";
...
function parseTemporalEditorValue(options) {
	...
	if (type === "time") {
		const parsed = _univerjs_core.numfmt.parseTime(content, {
			locale,
			dateSystem
		});
		return parsed ? Math.floor(originalSerial) + Number(parsed.v) : null;
	}
	const parsed = _univerjs_core.numfmt.parseDate(content, {
		locale,
		dateSystem
	});
		...
		this.disposeWithMe((0, _univerjs_core.toDisposable)(this._sheetInterceptorService.writeCellInterceptor.intercept(_univerjs_sheets.BEFORE_CELL_EDIT, { handler: (value, context, next) => {
			...
			*/
			...
			if (numfmtCell) {
				...
				if (!(rawCell === null || rawCell === void 0 ? void 0 : rawCell.f) && (rawCell === null || rawCell === void 0 ? void 0 : rawCell.t) === _univerjs_core.CellValueType.NUMBER && (0, _univerjs_core.isRealNum)(rawCell.v)) {
				...
				switch ((0, _univerjs_sheets_numfmt.getPatternType)(numfmtCell.pattern)) {
					/**
					...
					*/
					case "percent": {
						const cell = { ...context.worksheet.getCellRaw(row, col) };
						if ((cell === null || cell === void 0 ? void 0 : cell.t) === _univerjs_core.CellValueType.NUMBER && (0, _univerjs_core.isRealNum)(cell.v)) {
							cell.v = `${(0, _univerjs_engine_formula.stripErrorMargin)(Number(cell.v) * 100)}%`;
	...
	*/
	_initInterceptorEditorEnd() {
		this.disposeWithMe((0, _univerjs_core.toDisposable)(this._sheetInterceptorService.writeCellInterceptor.intercept(_univerjs_sheets.AFTER_CELL_EDIT, { handler: (value, context, next) => {
			...
			if ((currentNumfmtValue === null || currentNumfmtValue === void 0 ? void 0 : currentNumfmtValue.pattern) && (originCell === null || originCell === void 0 ? void 0 : originCell.t) === _univerjs_core.CellValueType.NUMBER && (0, _univerjs_core.isRealNum)(originCell.v)) {
				...
				if (temporalValue != null) return next({
					...value,
					p: void 0,
					v: (0, _univerjs_engine_formula.stripErrorMargin)(temporalValue, 16),
...
NumfmtEditorController = __decorate([
	...
	__decorateParam(4, (0, _univerjs_core.Inject)(_univerjs_sheets_numfmt.SheetsNumfmtCellContentController)),
	__decorateParam(5, (0, _univerjs_core.Optional)(_univerjs_sheets_ui.IEditorBridgeService))
