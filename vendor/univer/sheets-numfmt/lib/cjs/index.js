...
function getCurrencySymbolIconByLocale(locale) {
	switch (locale) {
		...
		case _univerjs_core.LocaleType.VI_VN: return {
			icon: "DongIcon",
			symbol: localeCurrencySymbolMap.get(locale) || "₫",
			locale
		};
		case _univerjs_core.LocaleType.AR_SA:
		case _univerjs_core.LocaleType.FA_IR: return {
			icon: "RialIcon",
			symbol: localeCurrencySymbolMap.get(locale) || "﷼",
			locale
		};
		case _univerjs_core.LocaleType.KO_KR: return {
			icon: "WonIcon",
			symbol: localeCurrencySymbolMap.get(locale) || "₩",
			locale
		};
		case _univerjs_core.LocaleType.ID_ID: return {
			icon: "RupiahIcon",
			symbol: localeCurrencySymbolMap.get(locale) || "Rp",
			locale
		};
		case _univerjs_core.LocaleType.PL_PL: return {
			icon: "ZlotyIcon",
