...
import { RangeProtectionPermissionEditPoint, SetSelectionsOperation, SheetsSelectionsService, UniverSheetsPlugin, WorkbookEditablePermission, WorksheetEditPermission, WorksheetSortPermission, expandToContinuousRange, getPrimaryForRange, getSheetCommandTarget } from "@univerjs/sheets";
...
import { SheetsRenderService, SheetsUIPart, UniverSheetsUIPlugin, getCurrentExclusiveRangeInterest$, getCurrentRangeDisable$ } from "@univerjs/sheets-ui";
...
let SheetsSortUIService = class SheetsSortUIService extends Disposable {
	...
	getTitles(hasTitle) {
		...
		return Array.from({ length: range.endColumn - range.startColumn + 1 }, (_, i) => {
			...
			return {
				...
				label: hasTitle ? `${cellValue ?? colTranslator(i + range.startColumn)}` : colTranslator(i + range.startColumn)
			};
		});
	}
	...
	async _detectSortLocation(extend) {
		...
		if (extend === true) {
			...
			if (confirmRes === "cancel") return null;
			if (confirmRes === "keep") range = selection.range;
			else {
				range = expandToContinuousRange(selection.range, {
					up: true,
					down: true,
		...
		children: [/* @__PURE__ */ jsxs("div", {
			...
			children: [/* @__PURE__ */ jsx("div", {
				...
				overlay: /* @__PURE__ */ jsx("ul", {
					className: clsx("univer-my-0 univer-box-border univer-grid univer-max-h-[310px] univer-w-[--radix-popper-anchor-width] univer-items-center univer-gap-1 univer-overflow-y-auto univer-overflow-x-hidden univer-rounded-lg univer-border univer-bg-gray-0 univer-p-1 univer-text-base univer-shadow-lg", scrollbarClassName),
				...
				children: /* @__PURE__ */ jsxs("div", {
					className: clsx("univer-ml-2 univer-flex univer-w-full univer-items-center univer-justify-between univer-overflow-hidden univer-rounded-md univer-py-1.5 univer-text-sm univer-text-gray-900 dark:!univer-text-gray-0"),
					children: [/* @__PURE__ */ jsx("span", {
						className: "univer-max-w-[220px] univer-truncate",
						children: itemLabel
...
//#endregion
export { menuSchema as SheetsSortUIMenuSchema, SortRangeAscCommand, SortRangeAscExtCommand, SortRangeAscExtInCtxMenuCommand, SortRangeAscInCtxMenuCommand, SortRangeCustomCommand, SortRangeCustomInCtxMenuCommand, SortRangeDescCommand, SortRangeDescExtCommand, SortRangeDescExtInCtxMenuCommand, SortRangeDescInCtxMenuCommand, UniverSheetsSortUIPlugin };