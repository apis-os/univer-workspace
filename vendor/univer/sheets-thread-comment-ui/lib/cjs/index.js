...
const OpenSheetCommentPanelOperation = {
	...
	type: _univerjs_core.CommandType.OPERATION,
	handler(accessor) {
		openSheetCommentPanel(accessor.get(_univerjs_ui.ISidebarService), accessor.get(_univerjs_thread_comment_ui.ThreadCommentPanelService));
		return true;
	}
};
...
const AddSheetDrawingCommentOperation = {
	...
	handler(accessor) {
		...
		accessor.get(_univerjs_thread_comment_ui.ThreadCommentDraftService).place({
			...
			anchor: {
				kind: _univerjs_thread_comment.ThreadCommentAnchorKind.SHEET_DRAWING,
...
const menuSchema = {
	...
	[_univerjs_ui.FloatingObjectToolbarPosition.SHEET]: { [AddSheetDrawingCommentOperation.id]: {
		order: 10,
		menuItemFactory: drawingCommentMenuFactory
	} },
	[_univerjs_ui.ContextMenuPosition.DRAWING]: { [_univerjs_ui.ContextMenuGroup.OTHERS]: { [AddSheetDrawingCommentOperation.id]: {
		order: 0,
		menuItemFactory: drawingCommentMenuFactory
	} } }
};
...
let SheetsThreadCommentPermissionController = class SheetsThreadCommentPermissionController extends _univerjs_core.Disposable {
	...
	_hasCommentPermission(unitId, subUnitId, ref) {
		...
		const anchor = (0, _univerjs_thread_comment.deserializeThreadCommentAnchor)(ref);
		if ((anchor === null || anchor === void 0 ? void 0 : anchor.kind) === _univerjs_thread_comment.ThreadCommentAnchorKind.SHEET_DRAWING) return this._sheetPermissionCheckController.permissionCheckWithoutRange(permissionTypes, unitId, subUnitId);
		const { row, column } = (0, _univerjs_engine_formula.singleReferenceToGrid)(ref);
...
let SheetsThreadCommentPopupController = class SheetsThreadCommentPopupController extends _univerjs_core.Disposable {
	...
	_initPanelListener() {
		this.disposeWithMe(this._threadCommentPanelService.activeCommentId$.subscribe(async (commentInfo) => {
			if (commentInfo) {
				...
				if ((anchor === null || anchor === void 0 ? void 0 : anchor.kind) === _univerjs_thread_comment.ThreadCommentAnchorKind.SHEET_DRAWING) return;
	...
	_initMarkSelection() {
		this.disposeWithMe((0, rxjs.combineLatest)([this._threadCommentPanelService.activeCommentId$, this._threadCommentPanelService.hoveredCommentId$]).pipe((0, rxjs.debounceTime)(100)).subscribe(([activeComment, hoveredComment]) => {
...
const SheetsThreadCommentCell = () => {
	...
	const rootId = (0, _univerjs_ui.useObservable)(activePopup ? () => sheetThreadCommentModel.commentUpdate$.pipe((0, rxjs.map)(() => getRootId()), (0, rxjs.startWith)(getRootId())) : null, void 0, false, [activePopup, sheetThreadCommentModel]);
...
const SheetsThreadCommentPanel = () => {
	...
	const draft = (0, _univerjs_ui.useObservable)(draftService.draft$, draftService.draft);
	const tempDrawingComment = (draft === null || draft === void 0 ? void 0 : draft.anchor.kind) === _univerjs_thread_comment.ThreadCommentAnchorKind.SHEET_DRAWING && draft.unitId === unitId ? {
		id: "",
		threadId: "",
		unitId: draft.unitId,
		subUnitId: draft.subUnitId,
		ref: (0, _univerjs_thread_comment.serializeThreadCommentAnchor)(draft.anchor),
		dT: "",
	...
	const showShape = (0, react.useCallback)((comment) => {
		if ((0, _univerjs_thread_comment.deserializeThreadCommentAnchor)(comment.ref)) return null;
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_univerjs_thread_comment_ui.ThreadCommentPanel, {
		...
		formatRef: (comment) => {
			const anchor = (0, _univerjs_thread_comment.deserializeThreadCommentAnchor)(comment.ref);
			return (anchor === null || anchor === void 0 ? void 0 : anchor.kind) === _univerjs_thread_comment.ThreadCommentAnchorKind.SHEET_DRAWING ? `#${anchor.elementId}` : comment.ref;
		}
	});
};
...
let SheetsThreadCommentDrawingRenderController = class SheetsThreadCommentDrawingRenderController extends _univerjs_core.RxDisposable {
	constructor(_context, _commandService, _drawingManagerService, _commentModel, _panelService, _themeService) {
		...
		this._overlay = new _univerjs_thread_comment_ui.ThreadCommentCanvasOverlay(SHEET_COMMENT_DRAWING_OVERLAY_KEY, {
			...this._getColors(),
			zoomRatio: 1,
			markers: [],
			underlines: []
		...
		this.disposeWithMe((0, _univerjs_core.toDisposable)(this._overlay.onPointerDown$.subscribeEvent((_event, state) => this._onOverlayPointerDown(state))));
		this.disposeWithMe((0, _univerjs_core.toDisposable)(this._context.engine.onTransformChange$.subscribeEvent(() => this._syncOverlay())));
	...
	_syncOverlay() {
		...
		this._commentModel.query({
			...
			anchorKinds: [_univerjs_thread_comment.ThreadCommentAnchorKind.SHEET_DRAWING],
			resolved: false
		}).forEach(({ root }) => {
...
SheetsThreadCommentDrawingRenderController = __decorate([
	...
	__decorateParam(4, (0, _univerjs_core.Inject)(_univerjs_thread_comment_ui.ThreadCommentPanelService)),
...
let UniverSheetsThreadCommentUIPlugin = class UniverSheetsThreadCommentUIPlugin extends _univerjs_core.Plugin {
	...
	onRendered() {
		this._renderManagerService.registerRenderModule(_univerjs_core.UniverInstanceType.UNIVER_SHEET, [SheetsThreadCommentDrawingRenderController]);
...
UniverSheetsThreadCommentUIPlugin = __decorate([
	(0, _univerjs_core.DependentOn)(_univerjs_engine_render.UniverRenderEnginePlugin, _univerjs_thread_comment.UniverThreadCommentPlugin, _univerjs_drawing.UniverDrawingPlugin, _univerjs_sheets.UniverSheetsPlugin, _univerjs_thread_comment_ui.UniverThreadCommentUIPlugin, _univerjs_sheets_thread_comment.UniverSheetsThreadCommentPlugin, _univerjs_sheets_ui.UniverSheetsUIPlugin),
...
UniverSheetsThreadCommentMobileUIPlugin = __decorate([
	(0, _univerjs_core.DependentOn)(_univerjs_engine_render.UniverRenderEnginePlugin, _univerjs_thread_comment.UniverThreadCommentPlugin, _univerjs_sheets.UniverSheetsPlugin, _univerjs_thread_comment_ui.UniverThreadCommentMobileUIPlugin, _univerjs_sheets_thread_comment.UniverSheetsThreadCommentPlugin, _univerjs_ui.UniverMobileUIPlugin, _univerjs_sheets_ui.UniverSheetsMobileUIPlugin),
	__decorateParam(1, (0, _univerjs_core.Inject)(_univerjs_core.Injector)),
	__decorateParam(2, (0, _univerjs_core.Inject)(_univerjs_core.ICommandService)),
	__decorateParam(3, _univerjs_engine_render.IRenderManagerService),
	__decorateParam(4, _univerjs_core.IConfigService)
...
exports.SheetsThreadCommentUIMenuSchema = menuSchema;
...
Object.defineProperty(exports, 'UniverSheetsThreadCommentMobileUIPlugin', {
  enumerable: true,
  get: function () {
    return UniverSheetsThreadCommentMobileUIPlugin;
  }
});
