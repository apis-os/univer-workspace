import { DependentOn, Disposable, ICommandService, IConfigService, IResourceManagerService, IUniverInstanceService, Inject, Injector, ObjectMatrix, Plugin, UniverInstanceType, generateRandomId, merge, sequenceExecuteAsync, toDisposable, touchDependencies } from "@univerjs/core";
...
import { AddCommentMutation, DeleteCommentMutation, IThreadCommentDataSourceService, TC_PLUGIN_NAME, ThreadCommentModel, UniverThreadCommentPlugin, UpdateCommentRefMutation, createThreadCommentResourceHook, deserializeThreadCommentAnchor } from "@univerjs/thread-comment";
...
let SheetsThreadCommentModel = class SheetsThreadCommentModel extends Disposable {
	...
	_addComment(unitId, subUnitId, comment) {
		if (deserializeThreadCommentAnchor(comment.ref)) return;
	...
	_initUpdateTransform() {
		this.disposeWithMe(this._threadCommentModel.commentUpdate$.subscribe((update) => {
			...
			switch (update.type) {
				...
				case "updateRef": {
					if (deserializeThreadCommentAnchor(update.payload.ref)) return;
...
let SheetsThreadCommentRefRangeController = class SheetsThreadCommentRefRangeController extends Disposable {
	...
	_initData() {
		...
		for (const data of datas) for (const thread of data.threads) {
			const { unitId, subUnitId, root } = thread;
			if (deserializeThreadCommentAnchor(root.ref)) continue;
...
let SheetsThreadCommentResourceController = class SheetsThreadCommentResourceController extends Disposable {
	...
	_initSnapshot() {
		this.disposeWithMe(this._resourceManagerService.registerPluginResource(createThreadCommentResourceHook(this._threadCommentModel, this._threadCommentDataSourceService, SHEET_UNIVER_THREAD_COMMENT_PLUGIN, [UniverInstanceType.UNIVER_SHEET])));
	}
	_initSheetChange() {
...
SheetsThreadCommentResourceController = __decorate([
	...
	__decorateParam(4, IThreadCommentDataSourceService)
...
//#endregion
export { SHEET_UNIVER_THREAD_COMMENT_PLUGIN, SheetsThreadCommentModel, SheetsThreadCommentRefRangeController, SheetsThreadCommentResourceController, UniverSheetsThreadCommentPlugin };