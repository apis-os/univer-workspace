...
import { CanceledError, ICommandService, IUniverInstanceService, Inject, Injector, RichTextBuilder, RichTextValue, Tools, UniverInstanceType, UserManagerService, generateRandomId } from "@univerjs/core";
...
var FRangeSheetsThreadCommentMixin = class extends FRange {
	...
	addCommentAsync(content, options = {}) {
		...
		if (content instanceof FTheadCommentBuilder) {
			...
			text = commentData.text;
		} else text = ThreadComment.normalizeThreadCommentContent(content);
		return commandService.executeCommand(ThreadComment.AddCommentCommand.id, {
			unitId,
			subUnitId: sheetId,
			comment: {
				text,
				dT: options.dateTime ? ThreadComment.getDT(options.dateTime) : (commentData === null || commentData === void 0 ? void 0 : commentData.dT) || ThreadComment.getDT(),
	...
	clearCommentAsync() {
		...
		const unitId = this._workbook.getUnitId();
		const sheetId = this._worksheet.getSheetId();
		if (currentComment) return commandService.executeCommand(ThreadComment.DeleteCommentTreeCommand.id, {
			unitId,
			subUnitId: sheetId,
	...
	async clearCommentsAsync() {
		...
		return (await Promise.all(this._getCommentDataInRange().map((comment) => commandService.executeCommand(ThreadComment.DeleteCommentTreeCommand.id, {
			unitId,
			subUnitId,
