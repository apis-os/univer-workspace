...
var FUniverSheetsUIMixin = class extends FUniver {
	...
	_initObserverListener(injector) {
		...
		const sheetAvailable$ = univerInstanceService.getAllUnitsForType(UniverInstanceType.UNIVER_SHEET).length > 0 ? of(true) : univerInstanceService.getTypeOfUnitAdded$(UniverInstanceType.UNIVER_SHEET).pipe(take(1));
		this.disposeWithMe(combineLatest([sheetAvailable$, lifeCycleService.lifecycle$]).pipe(filter(([, lifecycle]) => lifecycle >= LifecycleStages.Rendered), take(1)).subscribe(() => registerPointerEventListeners()));
		let sheetRenderUnit;
