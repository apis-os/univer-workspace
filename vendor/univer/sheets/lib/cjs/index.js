...
const TextToNumberCommand = {
	...
	handler: (accessor, params) => {
		...
		const { workbook, worksheet, unitId, subUnitId } = target;
		const parseOptions = {
			locale: (0, _univerjs_core.getNumfmtLocaleTag)(workbook.getSnapshot().locale),
		...
		for (let i = 0; i < ranges.length; i++) for (let r = ranges[i].startRow; r <= ranges[i].endRow; r++) for (let c = ranges[i].startColumn; c <= ranges[i].endColumn; c++) {
			...
			const parsedValue = typeof (cell === null || cell === void 0 ? void 0 : cell.v) === "string" ? (0, _univerjs_core.getNumfmtParseValueFilter)(cell.v, parseOptions) : null;
