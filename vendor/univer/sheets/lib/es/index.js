...
const SetSelectionsOperation = {
	...
	handler: (accessor, params) => {
		...
		getSelectionsService(accessor, fromCurrentSelection).setSelections(unitId, subUnitId, [...selections], type);
		return true;
	}
};
...
const TextToNumberCommand = {
	...
	handler: (accessor, params) => {
		...
		for (let i = 0; i < ranges.length; i++) for (let r = ranges[i].startRow; r <= ranges[i].endRow; r++) for (let c = ranges[i].startColumn; c <= ranges[i].endColumn; c++) {
			...
			const parsedValue = typeof (cell === null || cell === void 0 ? void 0 : cell.v) === "string" ? getNumfmtParseValueFilter(cell.v, parseOptions) : null;
